"""Finite saved-source controls/freeze/execution. No real requests or solver."""
import argparse
...
PROTOCOL=ROOT/'docs/parsed_source_reuse_protocol_20260925_r1.md'
GATE=ROOT/'docs/parsed_source_reuse_controls_20260925_r1.json'
CONFIG=ROOT/'configs/backend_controls/parsed_source_reuse_r1.json'
DEST=ROOT/'data/moe/results/parsed_source_reuse_20260925_r1'
PARENT=ROOT/'docs/source_cost_supervised_audit_20260925_r1.json'


def sources():
    prior=load(ROOT/'configs/backend_controls/source_cost_supervised_r1.json')['sources']
    for path,digest in prior.items():
        if sha(ROOT/path)!=digest:raise ValueError('sealed source changed: '+path)
    paths=[PROTOCOL,*sorted((ROOT/'parsed_source_reuse').glob('*.py'))]
    return {**prior,**{str(p.relative_to(ROOT)):sha(p) for p in paths}}


def inputs():
    cfg=load(ROOT/'configs/backend_controls/source_cost_supervised_r1.json');parent=load(PARENT)
    if parent['audit']!='PASS' or parent['completed_profiles']!=2:raise ValueError('valid saved diagnosis prerequisite')
    out=[]
    for spec,row in zip(cfg['calls'],parent['results']):
        if row['id']!=spec['id'] or row['status']!='COMPLETED':raise ValueError('saved scope mismatch')
        folder=Path(cfg['output'])/spec['id']/'profile';p=row['profile']
        method={'schema':'PARSED_SOURCE_SAVED_CHECK_R1','enabled':False,'expected_result_sha256':identity(p['source_check'])}
        for name in ('source','construction'):
            path=folder/(name+'.json');record=p['files'][name]
            if path.is_symlink() or sha(path)!=record['sha256'] or path.stat().st_size!=record['bytes']:
                raise ValueError('saved source changed')
            method[name]={**record,'path':str(path)}
        out.append({'spec':spec,'method':method})
    return out
    ...
    for item in inputs():
        for repeat in range(3):
            for enabled in ((False,True) if repeat%2==0 else (True,False)):
                name=item['spec']['id']+'_'+str(repeat)+('_on' if enabled else '_off')
                out.append({'id':name,'spec':dict(item['spec'],id=name),'repeat':repeat,
                            'fixture_id':item['spec']['id'],'method':dict(item['method'],enabled=enabled)})
    return out
...
def controls(root):
    ...
    if GATE.exists() or not root.is_relative_to(ROOT/'data/moe/results') or not admitted():
        raise ValueError('new control path/resource gate required')
    ...
    groups=[('new',['parsed_source_reuse.tests','parsed_source_reuse.supervision_tests'],27),
...
def freeze():
    ...
    if gate['status']!='PASS' or gate['tests']!=207 or gate['sources']!=sources():raise ValueError('test/source gate')
    if CONFIG.exists() or DEST.exists():raise FileExistsError('new freeze and output required')
    save(CONFIG,{'schema':'PARSED_SOURCE_SAVED_CHECK_STUDY_R1','sources':sources(),'gate_sha256':sha(GATE),
        ...
        'scope':'saved-source checking ONLY; not production MoE latency or new output certification'})
    print({'frozen_calls':12,'config_sha256':sha(CONFIG),'executed':False})


def verify():
    c=load(CONFIG);gate=load(GATE,c['gate_sha256'])
    if (c['sources']!=sources() or gate['sources']!=c['sources'] or gate['status']!='PASS' or gate['tests']!=207
            ...
            or c['real_requests']!=0 or c['native_solver_queries']!=0):raise ValueError('frozen scope/environment drift')
    return c


def execute():
    from parsed_source_reuse.execution import supervise
    clean();cfg=verify()
