...
def guard(event,args):
    ...
    if event=='import' and args[0].split('.')[0] in ('torch','numpy','scipy','highspy','gurobipy','full_source','source_enclosure','source_ranges','range_pipeline','property_ranges'):
...
def verify(root,expected_hash):
    ...
    from selection_check import check as check_selection
    ...
    if m['schema']!='SELECTED_COMPLETE_EXPERT_LP_BUNDLE_V1' or m['endpoint']!='COMPLETE_SOURCE_AND_NEW_LP_CONSTRUCTIONS_NOT_POSITIVITY':
        raise ValueError('endpoint is construction only')
    policy=m['policy']
    if policy.get('selection') not in ('prefix','property') or policy!={'selection':policy['selection'],'layer':6,'rows_per_expert':2,'native_seconds':3.}:
    ...
    if trace['schema']!='SELECTED_COMPLETE_EXPERT_TRACE_V1' or [(v['expert'],v['layer']) for v in trace['steps']]!=expected:
    ...
    required={'full_experts.json','trace.json','joint.state.json','lp_base.json','obligations.json',
              'full_graph.py','delta_format.py','lift_check.py','obligation_check.py','verify_full.py','range_check.py','range_step_check.py','selection_check.py'}
        ...
        for index in ['coordinate']+list(range(2,9)):
            ...
            if kind=='Flatten':
                ...
                if kind=='ReLU':checked=check_relu(old,state,row['proof'],tag)
                    ...
                    name=f'expert{i}_selection.json'
                    if row.get('selection_file')!=name:raise ValueError('selection file scope')
                    required.add(name)
                    selected=check_selection(old,op,bias,expert['layers'][8],ctx,source['request']['classes'],
                                             source['request']['clean_prediction'],policy['selection'],load(name))
                    if len(facts)!=len(op) or any(f is not None and j not in selected for j,f in enumerate(facts)):
    ...
    return {'status':'CHECKED_SELECTED_FULL_EXPERTS_AND_NEW_LP_CONSTRUCTIONS','range_policy':policy,
