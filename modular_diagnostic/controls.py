...
def run():
    ...
    while (ROOT/f'docs/modular_diagnostic_controls_attempt{n:03}.json').exists():n+=1
    result=unittest.TextTestRunner(stream=log,verbosity=2).run(unittest.defaultTestLoader.loadTestsFromNames([
        'modular_diagnostic.tests','sparse_diagnostic_archive.tests']))
    C.verify_sealed()
    if before!=C.sources():raise ValueError('control source drift')
    from modular_diagnostic.tests import ARTIFACT_ROOT,JOBS,VALID_BATCHES
    ...
    destination=ROOT/f'docs/modular_diagnostic_controls_attempt{n:03}.json'
...
def review(controls,destination):
    ...
    moved=Path(tempfile.mkdtemp(prefix='modular_diagnostic_review_',dir=ROOT/'data/moe/results'))
    checks=[]
