...
def dynamic(system, p, b, stats, *, capture=False):
    ...
    def observe():
        ...
        stats['peak_live_nnz'] = max(stats['peak_live_nnz'], source_nnz+active+retained+b.plan_units)
...
def replay(system,p,b,stats,plan):
    ...
    def observe():
        b.tick()
        live=source_nnz+active+retained+b.plan_units
...
def solve(system,p,b,stats,*,plan=None,scope='analytic',capture=True):
    for k,v in (('plan_builds',0),('plan_replays',0),('peak_plan_units',0),
                ('plan_invalidations',[])):
        stats.setdefault(k,v)
    # No process-global cache. Even RHS/scope changes invalidate source binding.
    b.plan_units=0
    binding=bind(system,scope,b) if capture or plan is not None else None
    if plan is not None:
        validate(plan,binding,len(system),b,stats)
        try:
            x=replay(system,p,b,stats,plan)
            if not field_residual(system,x,p,b,stats):
                raise PlanMismatch('field_residual_mismatch')
            stats['plan_replays']+=1
            return x,plan
        except PlanMismatch as exc:
            stats['plan_invalidations'].append({'prime':p,'reason':str(exc),
                                               'operations':b.operations})
            # The caller still owns the old immutable plan until this returns.
            # Charge old + newly captured indices during fallback construction.
            # Counters/deadline and retained plan storage must not be reset.
    x,initial,steps=dynamic(system,p,b,stats,capture=capture)
    if not field_residual(system,x,p,b,stats):raise ValueError('dynamic field residual')
    new=None
    if capture:
        b.where('plan','publish',prime=p)
        new=Plan(binding,initial,steps,identity(payload(binding,initial,steps)))
        b.tick();stats['plan_builds']+=1
    return x,new
