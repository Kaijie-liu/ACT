...
\date{Technical review draft, 25 September 2026}
\begin{document}
\maketitle
\begin{abstract}
...
A separate repaired MetaMoE author-checkpoint comparison gives nine author
numerical filters versus four ACT policy acceptances, with no ACT-only positive.
...
These findings limit the practical and formal claims of the method. The
separate class-separated top-1 author-model comparison below is an adapter
evaluation, not an expansion of the weighted top-2 theorem's empirical scope.
...
\input{results/submission_tables.tex}
...
\paragraph{RQ3: repaired MetaMoE author-model comparison.}
A separate 20-call follow-up uses ten already observed inputs (five CIFAR-10,
five MNIST) and the same author checkpoint, materialized inputs and properties
in both arms. The task is class-separated raw-score top-1 with 19 global
output margins, normalized-space radius $2/255$ clipped to $[-10,10]$, and a
300-second whole-request budget. It is not the main weighted top-2 cohort or
a pixel-radius comparison. The author arm runs its backend with a disclosed
lAs compatibility wrapper and strict route-invariance sufficient adapter,
not an unchanged author-tool execution. The original stopped run remains
separate; no old and new results are spliced.

Table~\ref{tab:metamoe} has four ACT policy acceptances versus nine author
numerical filters: four shared, zero ACT-only, and five author-only CIFAR-10
cases. All ACT positives are route-stable. The formerly crashing author
request now times out after seven checked metadata restorations and native
insertions; compatibility is improved, but no new certificate follows.
This observed cohort provides no ACT competitive advantage. Distinct evidence
grades are not a reason to dismiss the practical coverage deficit.

The smaller all-request ACT mean is driven by the unresolved author timeout.
On nine normally returned pairs ACT is 16.03 seconds slower on average;
on four common positives the means are 10.55 versus 7.17 seconds. Saved logs
record 116 excluded output violation regions and 93 local timeouts, out of
209 obligations. Repeated setup and short per-property budgets are observable;
the hard MNIST request also lacks completed route/base obligations. None of
this establishes a relaxation impossibility or a counterfactual success with
more time. No new solve or tuning is used in this diagnosis.
...
\paragraph{Where the later same-source pipeline stops.}
Seven separately frozen engineering calls on four observed MLP inputs
(4088, 4096, 4098, 4099) end in six timeouts and one sampled-memory limit.
All retain 252 original output obligations. None reaches a native output LP
query or publishes a complete retained expert/output construction; there are
zero new output bounds or complete positive proofs. In the latest shared-router
run, checked exclusions discharge 225 duties online, leaving 27 on three
potential pairs. Potential pairs do not establish actual route change.
Offline router checks on other stopped calls are not retroactive online
discharges. The last real in-memory construction returns at 282.95 seconds,
before a publication-stage resource stop; only about 15 seconds remain before
the work cutoff, with source and output checks still outstanding.

A subsequent four-call synthetic control completes fresh upstream construction
and checking, but intentionally generates no output bounds. Readonly parsing
lowers whole cost on one object (0.241 to 0.222 seconds) and increases it on
the other (0.407 to 0.438 seconds), with one observation per arm. It remains
optional, with no real-request speed or proof-completion claim. These records
locate an execution frontier before output solving; they do not diagnose
insufficient output relaxation or justify another sample automatically.
The saved-only proof-frontier ledger accompanies the artifact; it is accounting,
not an independent reproof or an additional confirmation cohort.
...
MetaMoE studies compositional MoE verification~\cite{metamoe}. Our historical
route-invariance baseline is a labeled reimplementation. The new, separate
author-checkpoint/backend comparison uses an explicitly repaired sufficient
adapter and favors the author path; these are not the same experiment.
Route changes not necessarily implying output errors are not claimed as a
new observation.
...
relational precision benefit. A cheaper external path, the adverse MetaMoE
author-path comparison, negative convolutional transfer, and an exact
source-containment obstruction limit stronger claims.
