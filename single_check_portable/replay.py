...
from single_check_portable.execution import supervise, audit_outer, read, save_new
from single_check_portable.controls import hashes
...
OUT = ROOT/'data/moe/results/single_check_saved114_20260916_v3'


def run():
    from scripts.optional_evidence_dev_contract import git
    from evidence_cohort.run import resource, resource_ok
    controls_path = sorted((ROOT/'docs').glob('single_check_v3_controls_attempt*.json'))[-1]
    ...
    previous = ROOT/'docs/cached_portable_v2_saved114.json'
    if digest(previous.read_bytes()) != 'c8ce46cdce1172885c866143833e7cc8893dacabc5fecafede49ffa9c8bb945f':
        raise ValueError('V2 reference changed')
    ...
    started = tail_started-220  # inherited original reserve, no actual upstream computation
    ...
    record = {'schema': 'SINGLE_CHECK_SAVED114_V3', 'head': head,
              ...
              'observed_tail_including_outer_publication_seconds': ended-tail_started,
              'simulated_upstream_seconds': 220, 'observed_request_clock_seconds': ended-started,
              ...
              'promotion': False, 'previous_v2_sha256': digest(previous.read_bytes()),
              'precheck_files_absent': not (OUT/'tail/precheck.json').exists() and not (OUT/'tail/precheck.log').exists(),
              'files': {str(p.relative_to(OUT)): digest(p.read_bytes()) for p in OUT.rglob('*') if p.is_file()}}
    for name in ('candidate.json', 'packing.json'):
        path = OUT/'tail'/name
        if path.exists():
            value = read(path)
            record[name] = value
    ...
    save_new(ROOT/'docs/single_check_v3_saved114.json', record)
