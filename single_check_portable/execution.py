"""Single-full-check optional evidence-tail execution; no cohort selection or model execution.
...
def verdict(stages, output, elapsed, packing):
    ...
    if (not output['isolated'] or not output['site_disabled'] or output['solver_imported'] or
            ...
            output['cache']['enabled'] != packing['cache_enabled'] or
            packing.get('decision') != 'PACK_ONLY_THEN_ISOLATED_FULL_CHECK' or
            output.get('decision') != packing['decision'] or
            packing['mathematical_precheck_executed'] is not False or packing['checked_result'] is not None):
...
def publish(root, terminal, started, *, clock=time.monotonic, write=save_new):
    ...
    receipt = {'schema': 'SINGLE_CHECK_PORTABLE_ADMISSION_V1',
...
def run(source, request, destination, *, started, enabled=False):
    ...
    job = {'schema': 'SINGLE_CHECK_PORTABLE_TAIL_V1', 'source': str(source), 'request': request,
    ...
    try:
        ...
        for name in STAGES:
            ...
            if name == 'check':
                ...
                cmd = [ACT, '-m', 'single_check_portable.worker', name, str(root)]
        ...
        output = read(root/'check.log') if len(stages) == 2 and stages[-1]['state'] == 'COMPLETED' else None
    ...
    terminal = {'schema': 'SINGLE_CHECK_PORTABLE_TAIL_TERMINAL_V1', 'status': status,
...
def audit(root):
    ...
    if output is not None:
        ...
        if (output['deadline_monotonic'] != job['started_monotonic']+298 or
                ...
                meta.get('decision') != 'PACK_ONLY_THEN_ISOLATED_FULL_CHECK' or 'expected_result' in meta or
                meta['source_manifest_sha256'] != job['manifest_sha256'] or
                meta['manifest']['sha256'] != job['manifest_sha256'] or
                packing['source_manifest_sha256'] != job['manifest_sha256']):
            raise ValueError('clock/request/result binding mismatch')
        from single_check_portable.structure import check_result
        check_result(output['result'], meta)
...
def supervise(source, request, destination, *, started, enabled=False):
    ...
    try:
        ...
        with (root/'driver.log').open('xb') as log:
            p = subprocess.Popen([ACT, '-m', 'single_check_portable.execution', '--driver', str(root/'plan.json')],
    ...
    value = {'schema': 'SINGLE_CHECK_PORTABLE_OUTER_V1', 'status': status, 'budget_seconds': 300,
