"""Moved stdlib-only complete expert/LP construction checker, NOT a SAFE oracle."""
import argparse
...
def guard(event,args):
    ...
    if event=='import' and args[0].split('.')[0] in ('torch','numpy','scipy','highspy','gurobipy','full_source','source_enclosure'):
...
def verify(root,expected_hash):
    ...
    if m['schema']!='COMPLETE_EXPERT_OUTPUT_LP_BUNDLE_V1' or m['endpoint']!='COMPLETE_EXPERTS_AND_NEW_OUTPUT_LP_CONSTRUCTIONS_NO_BOUNDS':
    ...
    if trace['schema']!='COMPLETE_EXPERT_TRACE_V1' or [(v['expert'],v['layer']) for v in trace['steps']]!=expected:
    ...
    required={'full_experts.json','trace.json','joint.state.json','lp_base.json','obligations.json',
              'full_graph.py','delta_format.py','lift_check.py','obligation_check.py','verify_full.py'}
    ...
    return {'status':'CHECKED_FULL_EXPERTS_AND_NEW_LP_CONSTRUCTIONS','prefix_steps_checked':prefix['checked_steps'],
