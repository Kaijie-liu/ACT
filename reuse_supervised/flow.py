...
def validate_reuse_report(root, plan, candidate):
    ...
    for name in ('source_cache','matrix_cache'):
        cache=report[name]
        if cache['enabled'] is not enabled or any(v!=0 for k,v in cache.items() if k.startswith('live_')):
    ...
    try:
        ...
        for name in ('capture', 'propose'):
            ...
            with (root/(name+'.log')).open('xb') as log:
                proc = subprocess.Popen([ACT, '-m', 'reuse_supervised.worker', name,
...
def supervise(request, arm, destination, *, started):
    ...
    try:
        ...
        with (root/'driver.log').open('xb') as log:
            proc = subprocess.Popen([ACT, '-m', 'reuse_supervised.flow', str(root)],
    ...
    value = {'schema': 'UPSTREAM_REUSE_SUPERVISED_V1', 'status': status,
