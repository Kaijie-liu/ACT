...
import unittest
import sys
from reuse_supervised.study import ROOT,hashes,verify_old
from single_check_portable.execution import save_new


def run():
    verify_old();before=hashes();number=1
    while (ROOT/f'docs/reuse_supervised_controls_attempt{number:03}.json').exists():number+=1
    modules=['reuse_supervised.tests','upstream_reuse.tests','upstream_portable.tests',
    ...
    module=sys.modules.get('reuse_supervised.tests')
    ...
    path=ROOT/f'docs/reuse_supervised_controls_attempt{number:03}.json'
