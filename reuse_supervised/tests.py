...
from reuse_supervised.flow import supervise, audit, review_candidate, phase_state
...
class FullFlowControls(unittest.TestCase):
    def test_actual_two_arm_upstream_and_tamper(self):
        ...
        with request_case() as (base,req):
            for arm in ('reuse_off','reuse_on'):
                ...
                self.assertEqual(metrics['matrix_cache']['enabled'],arm=='reuse_on')
                self.assertEqual(metrics['source_cache']['enabled'],arm=='reuse_on')
    ...
    def test_expired_clock_no_capture_and_no_retry(self):
        with request_case() as (base,req):
            root=base/'expired';v=supervise(req,'reuse_on',root,started=time.monotonic()-299)
            self.assertEqual(v['status'],'TIMEOUT'); self.assertEqual(audit(root)['status'],'TIMEOUT')
            self.assertFalse((root/'source').exists())
            with self.assertRaises(FileExistsError):supervise(req,'reuse_on',root,started=time.monotonic())

    def test_invalid_model_binding_is_error_not_proof(self):
        with request_case() as (base,req):
            req['subject']['checkpoint_sha256']='0'*64;root=base/'bad'
            result=supervise(req,'reuse_on',root,started=time.monotonic())
    ...
    def test_process_and_arm_controls(self):
        ...
        with self.assertRaises(ValueError):supervise({},'reuse_on',ROOT/'unused',started=time.monotonic()+100)

    def test_roster_rotation_error_stop_and_denominator(self):
        from reuse_supervised.study import jobs,loop
        roster=jobs([{'dataset_index':i} for i in range(4)])
        self.assertEqual(len(roster),8)
        self.assertEqual([r['arm'] for r in roster[:4]],['reuse_off','reuse_on','reuse_on','reuse_off'])
        calls=[];published=[]
        def run(job):
            calls.append(job)
    ...
    def test_deadline_between_upstream_and_tail_is_timeout(self):
        with request_case() as (base,req):
            ...
            save_new(root/'plan.json',{'request':req,'arm':'reuse_on','started':1.})
    ...
    def test_real_outer_kill_no_clock_refund(self):
        with request_case() as (base,req):
            root=base/'short';out=supervise(req,'reuse_on',root,started=time.monotonic()-297.95)
    ...
    def test_terminal_publication_overrun_is_timeout(self):
        import reuse_supervised.flow as flow
        with request_case() as (base,req):
    ...
    def test_worker_refuses_unknown_arm_and_phase(self):
        ...
        with self.assertRaises(ValueError):execute('bad',ROOT,EvidenceBudget(time.monotonic()),'reuse_off')

    def test_missing_cost_not_zero_and_outcome_cost_separate(self):
        from reuse_supervised.study import paired_summary
        rows=[{'rank':0,'arm':'reuse_off','status':'TIMEOUT','complete_independent_check':False,
               'costs':{'whole_request_seconds':300}},
              {'rank':0,'arm':'reuse_on','status':'UNKNOWN_NONPOSITIVE','complete_independent_check':True,
