...
def old():
    from primitive_diagnostic.contract import verify
    verify()
    v=read(ROOT/'docs/primitive_diagnostic_v1_execution_results.json')
...
def sources():
    paths=list(Path(__file__).parent.glob('*.py'))+[ROOT/p for p in
       ('primitive_basis/engine.py','primitive_basis/tests.py','primitive_basis/review.py',
        ...
        'single_check_portable/execution.py','portable_proof/runtime.py','docs/modular_basis_v1.md',
        'docs/primitive_diagnostic_v1_execution_results.json')]
    return {str(p.relative_to(ROOT)):digest(p.read_bytes()) for p in sorted(paths)}


def run():
    old();before=sources();n=1
    while (ROOT/f'docs/modular_basis_controls_attempt{n:03}.json').exists():n+=1
    ...
    from modular_basis.tests import OBSERVATIONS,ARTIFACT_ROOT
    ...
    path=ROOT/f'docs/modular_basis_controls_attempt{n:03}.json';save_new(path,receipt)
