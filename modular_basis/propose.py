...
from modular_basis.engine import POLICY, Budget, statistics, eliminate, Unresolved
...
def propose(lp, statement, candidate, hint, expected_statement, expected_hint, *, deadline):
    ...
    return {'schema': 'MODULAR_BASIS_PROPOSAL_V1', 'status': status, 'error': error,
            'bundle': bundle, 'row_residuals': residuals, 'stats': stats, 'operations': b.operations,
            'policy': dict(POLICY), 'arithmetic': b.arithmetic, 'seconds': time.monotonic() - begin, 'deadline_monotonic': deadline,
