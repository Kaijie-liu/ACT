...
This directory contains the core verification framework for the the framework system. It implements a modern three-tier architecture: Front-End (data/model/spec processing), Back-End (verification core), and Pipeline (testing/integration) with PyTorch-native verification capabilities.

## Recent Development Updates

### Unified CLI Architecture (November 2025)
- **Shared CLI Utilities**: Created `cuc/util/cli_utils.py` with `add_device_args()` and `initialize_from_args()` for consistent device/dtype handling
- **Comprehensive CLI Coverage**: All modules now have dedicated CLIs with unified interface
  - `cuc.front_end` - Main front-end CLI and loader-specific CLIs
  - `cuc.pipeline` - Pipeline testing and integration CLI
  - `cuc.back_end` - Back-end verification and network generation CLI (NEW)
- **Architecture Separation**: Clean separation between native components (use cli_utils) and external verifiers (use local get_parser())
- **Back-End CLI**: New comprehensive CLI supporting network generation, verification, inspection, and serialization testing
- **Deprecated Code Removal**: Removed `cuc/main.py` in favor of modern modular CLI architecture
...
- **Transfer Function Logging**: Detailed layer-by-layer analysis logging to `cuc/pipeline/log/cuc_debug_tf.log`
...
- **Path Management**: Centralized logging to `cuc/pipeline/log/` using `path_config.py`
...
│   ├── __main__.py                 # Entry point for python -m cuc.back_end
...
│   │   ├── config_gen_cuc_net.yaml # Generator configuration for random nets
│   │   ├── nets/                   # Generated Net JSON files
...
│   ├── log/                        # Test execution logs (includes cuc_debug_tf.log)
...
- **`back_end/cli.py`**: Comprehensive back-end verification CLI
  ...
  - Unified device/dtype arguments via `cli_utils`
  - Usage: `python -m cuc.back_end [options]`
  - Examples:
    ```bash
    # Generate all example networks
    python -m cuc.back_end --generate --device cpu --dtype float64
    
    # List available networks
    python -m cuc.back_end --list-examples
    
    # Show network info
    python -m cuc.back_end --info --network mnist_robust_easy.json --verbose
    
    # Run verification
    python -m cuc.back_end --verify --network mnist_robust_easy.json --device cpu
    
    # Test serialization
    python -m cuc.back_end --test-serialization --device cpu
    ```
...
- **`wrapper_exts/ext_runner.py`**: External verifier CLI (ERAN, αβ-CROWN)
  ...
  - Separate from native CLI architecture
  - Usage: `python cuc/wrapper_exts/ext_runner.py [options]`

#### **Shared CLI Utilities**
- **`util/cli_utils.py`**: Shared CLI infrastructure for all native modules
...
- **`verifiable_model.py`**: PyTorch verification wrapper modules
  ...
  - `InputSpecLayer`: Wraps the framework InputSpec as nn.Module for seamless integration
  - `OutputSpecLayer`: Wraps the framework OutputSpec as nn.Module for property specification
...
- **`core.py`**: Fundamental the framework data structures
...
- **`verifier.py`**: Spec-free verification engine
  - `verify_once()`: Single-shot verification using embedded the framework constraints
  ...
  - No external input specs required - all constraints extracted from Net
...
- **`net_factory.py`**: YAML-driven network factory
  - Generates example networks from YAML configurations
...
- **`examples/`**: Example networks and test cases
  - **`config_gen_cuc_net.yaml`**: Generator configuration for example networks
  - **`nets/`**: Generated Net JSON files (MNIST, CIFAR, control, reachability)
...
- **`torch2cuc.py`**: Automatic PyTorch→Net conversion
  - Seamless conversion from PyTorch nn.Module to Net representation
...
- **`model_factory.py`**: Net factory for test networks
  - Pre-loads networks from `cuc/back_end/examples/nets/`
  - PyTorch model generation from Nets
...
- **Testing Framework**: Comprehensive validation and regression testing
  ...
  - **`integration.py`**: Front-end integration bridge for real components testing
...
- **`log/`**: Centralized logging directory
  - **`cuc_debug_tf.log`**: Transfer function debug output (layer-by-layer analysis, bounds, constraints)
...
- **`cli_utils.py`**: Shared CLI utilities for unified device/dtype handling
  ...
  - Used by all native modules (front_end, pipeline, back_end)
  - Ensures consistent CLI interface across the entire the framework
...
- **`path_config.py`**: Project path configuration and management
  ...
  - `get_pipeline_log_dir()`: Returns absolute path to `cuc/pipeline/log/`
  - `ensure_gurobi_license()`: Automatic Gurobi license detection
  - Centralized path management for all modules
...
- **`options.py`**: Performance configuration (PerformanceOptions only)
  - **`PerformanceOptions`**: Global debugging and performance flags
    ...
    - `debug_output_file`: Path to debug log (default: `cuc/pipeline/log/cuc_debug_tf.log`)
...
- **`abcrown_verifier.py`**: αβ-CROWN wrapper and interface
  - Translates the framework parameters to αβ-CROWN format
...
- **`abcrown_runner.py`**: αβ-CROWN backend execution script
  - Contains code adapted from the open-source αβ-CROWN project with enhancements for the framework integration
  - Executes within isolated `cuc-abcrown` conda environment
  - Direct interface to αβ-CROWN complete verification engine
  - Independent execution without the framework path dependencies
...
- **Pipeline Integration**: Comprehensive testing framework and Torch→conversion bridge
- **Clean Boundaries**: Clear interfaces between data processing, verification, and testing
- **Unified CLI**: Consistent device/dtype handling across all native modules via `cli_utils`
...
- **Clean Separation**: native components use `cli_utils`, external verifiers use local `get_parser()`
- **Entry Points**: All CLIs executable via `python -m cuc.<module>` pattern
...
- **Automatic Conversion**: Seamless PyTorch→Net conversion preserving all semantics
...
- **Centralized Logging**: All debug output to `cuc/pipeline/log/` with configurable detail levels
...
- **Integration Testing**: Real components testing with front-end bridge
...
- **Consistent Interface**: All native CLIs accept `--device {cpu,cuda,gpu}` and `--dtype {float32,float64}`
...
- **Parameter Translation**: Automatic conversion between the framework and backend-specific formats
...
python -m cuc.front_end.torchvision_loader --list --device cpu

# List available VNNLIB files
python -m cuc.front_end.vnnlib_loader --list --device cpu

# Create specifications from TorchVision
python -m cuc.front_end --create --dataset mnist --model resnet18 --device cuda
```

### Pipeline CLI Examples
```bash
# Run pipeline tests
python -m cuc.pipeline --test --device cpu --dtype float32

# Run validation
python -m cuc.pipeline --validate --device cuda
```
...
python -m cuc.back_end --info --network mnist_robust_easy.json --device cpu

# Display detailed network information
python -m cuc.back_end --info --network mnist_robust_easy.json --verbose --device cpu --dtype float64

# Run single-shot verification
python -m cuc.back_end --verify --network mnist_robust_easy.json --device cpu --dtype float64

# Run branch-and-bound verification
python -m cuc.back_end --verify --network mnist_robust_hard.json --bab --device cpu

# Test serialization round-trip
python -m cuc.back_end --test-serialization --device cpu --dtype float64
```

### External Verifier Examples
```bash
# Run ERAN verification
python cuc/wrapper_exts/ext_runner.py --verifier eran --model model.onnx --dataset mnist

# Run αβ-CROWN verification
python cuc/wrapper_exts/ext_runner.py --verifier abcrown --model model.onnx --dataset cifar10
```

## Examples and Network Generation
Example networks are stored as JSON under `cuc/back_end/examples/nets/`.
These files are generated from the generator configuration
`cuc/back_end/examples/config_gen_cuc_net.yaml` using the network factory.
...
The back-end CLI provides comprehensive tools for working with networks:

1. **Generate Networks**: `--generate` creates networks defined in `config_gen_cuc_net.yaml`
