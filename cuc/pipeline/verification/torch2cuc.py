# Spec-free PyTorch converter for verification. Converts wrapped
# PyTorch models (containing InputLayer, InputSpecLayer, and OutputSpecLayer)
# into Net graphs with embedded constraints for formal verification.
...
from cuc.back_end.layer_util import create_layer
from cuc.pipeline.verification.utils import _prod, _normalize_tuple

# Imports needed for main() test harness
from cuc.util.model_inference import model_inference
...
class _LayerGraphBuilder:
    """
    Build layer graph from nn.Module using torch.fx for graph extraction.
    ...
    def build_layer_graph(self) -> Tuple[List[Layer], Dict[int, List[int]], Dict[int, List[int]]]:
        """
        Build layer graph from the model.
    ...
    def _convert_module(self, mod: nn.Module) -> None:
        """Convert a PyTorch module to internal layer(s)."""
        converters = {
...
def build_act(
    ...
    """
    Build layer graph from a PyTorch model.
...
# -----------------------------------------------------------------------------
# TorchToCUC Converter
# -----------------------------------------------------------------------------

class TorchToCUC:
    """
    Convert a wrapped nn.Module to Net.
    ...
    def __init__(self, wrapped: nn.Module):
        if not isinstance(wrapped, nn.Module):
            raise TypeError("TorchToCUC expects an nn.Module.")
    ...
    def run(self) -> Net:
        """Convert wrapped PyTorch model to Net."""
        # Emit INPUT layer
        ...
        from cuc.back_end.layer_util import validate_graph
...
def main():
    """Main entry point for PyTorch conversion and verification testing."""
    # Initialize debug file (GUARDED)
    if PerformanceOptions.debug_tf:
        debug_file = PerformanceOptions.debug_output_file
        with open(debug_file, 'w') as f:
            f.write(f"Torch2CUC Conversion Debug Log\n")
            f.write(f"{'='*80}\n\n")
        print(f"Debug logging to: {debug_file}")
    
    print("Starting Spec-Free, Input-Free Verification Demo")
    ...
    try:
        ...
        gurobi_solver.begin("cuc_verification")
    ...
    try:
        ...
        torch_solver.begin("cuc_verification")
    ...
    for idx, (model_id, wrapped_model) in enumerate(successful_models.items(), 1):
        ...
        # === CONVERSION ===
        try:
            net = TorchToCUC(wrapped_model).run()
    ...
    print("\nConversion and verification completed!")
