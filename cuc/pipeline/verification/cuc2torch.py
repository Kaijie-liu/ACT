#!/usr/bin/env python3
# → PyTorch converter with spec-free verification support. Converts
# Net graphs (abstract constraint representations) into executable
...
from cuc.back_end.core import Net, Layer
from cuc.util.device_manager import get_default_dtype, get_default_device

logger = logging.getLogger(__name__)

class CUCToTorch:
    """
    Convert Net to PyTorch nn.Module.
    
    This class provides the inverse transformation of TorchToCUC, enabling
    bidirectional conversion between verification representations and
    executable models (PyTorch).
    
    Usage:
        converter = CUCToTorch(act_net)
        model = converter.run()  # Returns nn.Module
    
    Args:
        act_net: Net object containing layers with architecture and weights
    ...
    def __init__(self, act_net: Net):
        """
        Initialize converter with Net.

        Args:
            act_net: Net object (contains architecture + weights)

        Raises:
            TypeError: If act_net is not a Net instance
        """
        if not isinstance(act_net, Net):
            raise TypeError(f"CUCToTorch expects a Net object, got {type(act_net)}")
    ...
    def run(self) -> nn.Module:
        ...
        Iterates through layers, creates corresponding PyTorch layers,
    ...
    def _run_sequential_model(self) -> nn.Module:
        ...
        for i, act_layer in enumerate(self.act_net.layers):
            ...
            if kind == 'INPUT':
                ...
                from cuc.front_end.verifiable_model import InputSpecLayer
                from cuc.front_end.specs import InputSpec, InKind
                
                # Build InputSpec from layer
                kind_str = meta['kind']
                ...
                from cuc.front_end.verifiable_model import OutputSpecLayer
                from cuc.front_end.specs import OutputSpec, OutKind
                
                # Build OutputSpec from layer
                kind_str = meta['kind']
            ...
            # Build PyTorch layer from layer (includes weight transfer)
        ...
        if not torch_layers:
            raise ValueError("No valid PyTorch layers found in Net")
    ...
    def _transfer_weights(self, torch_layer: nn.Module, act_layer: Layer, 
        ...
        """
        Transfer weights and biases from layer to PyTorch layer.
        
        Args:
            torch_layer: PyTorch layer with weight/bias parameters
            act_layer: layer containing parameter tensors
        ...
        with torch.no_grad():
            ...
            # Transfer bias (or zero it if not present in layer)
    ...
    def _create_torch_layer(self, kind: str, meta: Dict[str, Any], 
        ...
        """
        Create PyTorch layer from layer kind and metadata.
        
        Args:
            kind: Layer kind string (DENSE, CONV2D, RELU, etc.)
            meta: Layer metadata dictionary
            act_layer: Optional Layer to load weights from
        ...
        if kind == "DENSE":
            ...
            # Transfer weights and bias from layer
