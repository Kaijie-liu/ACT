...
from cuc.pipeline.verification.per_neuron_bounds import PerNeuronCheckConfig, run_per_neuron_bounds_check
from cuc.back_end.verifier import verify_once, gather_input_spec_layers, seed_from_input_specs, get_input_ids, get_assert_layer, find_entry_layer_id
from cuc.util.stats import VerifyStatus
from cuc.back_end.solver.solver_gurobi import GurobiSolver
from cuc.back_end.solver.solver_gurobi import is_gurobi_available
from cuc.back_end.solver.solver_torch import TorchLPSolver
from cuc.util.options import PerformanceOptions
from cuc.front_end.specs import OutKind
...
class VerificationValidator:
    ...
    def __init__(
        ...
        if PerformanceOptions.debug_tf:
            ...
            with open(debug_file, 'w') as f:
                f.write(f"Verification Debug Log\n")
    ...
    def validate_counterexamples(
        ...
        """
        CBR: Validate verifier soundness using concrete counterexamples.
    ...
    def _validate_counterexample_single(
        ...
        """
        Validate verifier correctness for a single network (CBR).
        
        Args:
            name: Network name from Net JSON files
        ...
        # Step 3: Run formal verifier on Net
    ...
    def _cross_validate_counterexample(
        ...
        """
        Cross-validate concrete inference vs formal verification (CBR).
    ...
    def validate_bounds(
        ...
        """
        BBL: Validate abstract bounds overapproximate concrete values.
    ...
    def _validate_bounds_single(
        ...
        """
        Validate bounds for a single network (BBL).
        ...
        # Step 1: Load Net and PyTorch model
        act_net = self.factory.get_act_net(name)
        model = self.factory.create_model(name, load_weights=True)
        model = model.to(device=self.device, dtype=self.dtype)
        ...
        for sample_idx in range(num_samples):
            ...
            from cuc.back_end.core import Fact, Bounds
    ...
    def validate_comprehensive(
        ...
        """
        Run both CBR and BBL validations.
        
        Args:
            networks: List of network names (None = all networks)
            solvers: List of solver names for CBR
            tf_modes: Transfer function modes for BBL
            num_samples: Number of samples for BBL
        ...
        logger.info(f"{'='*80}")
        logger.info(f"Running both CBR and BBL validation")
        logger.info(f"Device: {self.device}, Dtype: {self.dtype}")
        logger.info(f"{'='*80}\n")
    ...
    def _print_comprehensive_summary(self, combined: Dict[str, Any]):
        ...
        print(f"\nCBR: {l1['passed']}/{l1['total']} passed, {l1['failed']} failed, {l1['errors']} errors")
        print(f"BBL:         {l2['passed']}/{l2['total']} passed, {l2['failed']} failed, {l2['errors']} errors")
        print()
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Verification Validator')
    ...
    parser.add_argument('--solvers', nargs='+', default=['gurobi', 'torchlp'],
                       help='Solvers for CBR')
    parser.add_argument('--tf-modes', nargs='+', default=['interval'],
                       help='Transfer function modes for BBL')
    parser.add_argument('--samples', type=int, default=10,
                       help='Number of samples for BBL')
