#!/usr/bin/env python3
# Level-2 (per-neuron) numerical validation. This module checks that the
...
from cuc.back_end.core import Bounds, Layer
...
def compute_abstract_bounds(
    ...
    *,
    ...
    """
    Compute abstract bounds for all layers in the net.
    """
    errors: List[str] = []
...
def collect_concrete_activations(
    ...
    *,
    ...
    """
    Collect concrete activations and align them to layer IDs.
    """
    errors: List[str] = []
    warnings: List[str] = []
    ...
    hookable_kinds = set(_CUC_KIND_TO_MODULE.values())
    ...
    hookable_layers = [
        ...
        if _CUC_KIND_TO_MODULE.get(L.kind) in hookable_kinds
    ]
    ...
    for idx, layer in enumerate(hookable_layers):
        ...
        expected = _CUC_KIND_TO_MODULE.get(layer.kind)
        if expected is None:
            errors.append(
                f"Unsupported layer kind at position {idx}: act_kind={layer.kind}"
            )
