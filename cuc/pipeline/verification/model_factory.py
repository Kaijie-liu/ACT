...
from cuc.back_end.serialization.serialization import NetSerializer
from cuc.pipeline.verification.cuc2torch import CUCToTorch
from cuc.util.device_manager import get_default_dtype, get_default_device
from cuc.util.path_config import get_examples_nets_dir
...
class ModelFactory:
    """Factory for creating PyTorch models from Net JSONs."""
    ...
    def __init__(
        ...
        Args:
            nets_dir: Directory containing pre-generated Net JSON files
            manifest_path: Optional path to manifest.json listing network names
        """
        self.nets_dir = Path(nets_dir)
        self.manifest_path = Path(manifest_path) if manifest_path else None
        
        # Pre-load all Nets for fast access (avoids repeated file I/O)
    ...
    def _load_all_nets(self) -> None:
        """
        Pre-load all Nets from JSON files at initialization.
        ...
        for name in self.net_names:
            ...
            if not net_path.exists():
                logger.warning(f"Net file not found: {net_path}. Skipping '{name}'.")
                continue
            ...
            try:
                ...
                logger.debug(f"Pre-loaded Net '{name}' from {net_path}")
            except Exception as e:
                logger.error(f"Failed to load Net '{name}' from {net_path}: {e}")
                continue
        
        logger.info(f"Pre-loaded {len(self.nets)} Nets from {self.nets_dir}")
    
    def get_act_net(self, name: str) -> Net:
        """
        Get pre-loaded Net by name.
        ...
        if name not in self.nets:
            ...
            raise KeyError(f"Net '{name}' not available. Available: {available}")
    ...
    def create_model(self, name: str, load_weights: bool = True) -> nn.Module:
        ...
        Args:
            name: Network name from examples_config.yaml
            load_weights: If True, load weights from corresponding Net JSON file
