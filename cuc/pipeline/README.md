# Pipeline Module# Pipeline Testing Framework

PyTorch model generation, conversion, and testing utilities for the the framework framework.A comprehensive testing framework for validating the the framework abstraction verifier with configurable mock testing, property-based validation, and regression testing capabilities.

## Overview## Overview

The Pipeline module provides tools for:The Pipeline Testing Framework provides a robust testing infrastructure to validate the correctness, performance, and reliability of the abstraction verifier. It supports multiple testing methodologies including mock input generation, real dataset integration, regression testing, and performance profiling.

- **PyTorch Model Generation**: Create PyTorch models from YAML configurations with exact weight loading

- **PyTorch ↔ conversion**: Convert between PyTorch nn.Module and Net representations## Architecture
...
cuc/pipeline/├── regression.py         # Baseline capture and regression testing
...
├── torch2cuc.py          # PyTorch→Net converter├── utils.py              # Shared utilities and performance profiling
...
├── regression.py         # Baseline management and regression testing├── torch2cuc.py          # Torch→the framework converter 
...
- **torch2cuc.py** - PyTorch→Net conversion utilities
...
Create PyTorch models from `config_gen_cuc_net.yaml` with exact weight equivalence to Nets:- **Scenario Composition**: Combine different components into complete test scenarios

```python### 3. **Comprehensive Validation Coverage**

from cuc.pipeline import ModelFactory- **Correctness Testing**: Validate verifier results against expected outcomes
...
# Create model with exact weights from Net### 4. **Real-World Integration**

model = factory.create_model("mnist_mlp_small", load_weights=True)- **Front-End Bridge**: Integration with the framework's actual front-end loaders and specifications
...
- Loads Net JSON from `cuc/back_end/examples/nets/`- **Input Specifications**: L∞/L2 perturbations, box constraints, custom bounds
...
### 2. PyTorch → conversion```python

# Validate verifier correctness with comprehensive testing

Convert PyTorch models to Net representation:validator = AbstractionVerifierValidator()

result = validator.validate_correctness(test_cases)

```python```

from cuc.pipeline import TorchToCUC

**Validation Types:**

# Initialize converter- **Basic Correctness**: Expected SAT/UNSAT results match actual outcomes

converter = TorchToCUC()- **Property Testing**: Soundness (no false negatives) and completeness validation

- **Performance Testing**: Execution time, memory usage, resource consumption

# Convert PyTorch model to Net- **BaB Refinement**: Branch-and-bound refinement effectiveness testing

act_net = converter.convert(pytorch_model, input_shape=(1, 784))

### Regression Testing (`regression.py`)

# Use Net for verification```python

from cuc.back_end.verifier import verify_once# Capture baselines and detect regressions
...
from cuc.pipeline import ValidationResult, PerformanceResult, VerifyResult- **Regression Detection**: Automated detection of performance/correctness regressions
...
val_result = ValidationResult(### Integration Testing (`integration.py`)
    ...
    total_tests=100,# Test with real the framework front-end components
...
from cuc.pipeline import BaselineManager, RegressionTestergenerator = ReportGenerator()
...
regression_result = tester.compare_to_baseline(```python
    ...
    baseline_name="v1.0",from cuc.pipeline import validate_abstraction_verifier
...
from cuc.pipeline import quick_validate
...
- Customizable thresholdsfrom cuc.pipeline import MockInputFactory, AbstractionVerifierValidator
...
from cuc.pipeline import ReportGenerator, ReportConfigvalidator = AbstractionVerifierValidator()
...
config = ReportConfig(
    ...
    plot_format="png"from cuc.pipeline import BaselineManager, RegressionTester
...
from cuc.pipeline import PerformanceProfiler
...
from cuc.pipeline import (    type: "image"
...
### config_gen_cuc_net.yaml## Testing Workflow
...
networks:from cuc.pipeline import quick_validate
...
**Location**: `cuc/back_end/examples/config_gen_cuc_net.yaml````bash
...
python cuc/pipeline/model_factory.py    def validate(self, test_case: TestCase) -> ValidationResult:
...
Testing ModelFactory with all 4 networks from config_gen_cuc_net.yaml...validator.register_custom_validator("my_test", CustomValidator())
...
✓ PyTorch model output matches Net (diff = 0.00e+00)```python
...
✓ PyTorch model output matches Net (diff = 0.00e+00)        return generated_data
...
✓ PyTorch model output matches Net (diff = 0.00e+00)```

Testing network: custom_mlp_small### Custom Report Formats

✓ Model created successfully```python

✓ PyTorch model output matches Net (diff = 0.00e+00)class CustomReportGenerator:
...
python -c "from cuc.pipeline import ModelFactory, TorchToCUC, ValidationResult, VerifyResult; print('✓ Import successful!')"
...
### Optional Featureslogging.getLogger('cuc.pipeline').setLevel(logging.DEBUG)

- `matplotlib` - Plotting and visualization (for reporting)

- `seaborn` - Enhanced visualizations (for reporting)# Memory debugging

from cuc.pipeline.verification.utils import print_memory_usage
...
## Integration with the framework- **Resource Management**: Intelligent resource allocation and cleanup
...
The pipeline integrates with the framework's spec creators for efficient testing:
...
```python
from cuc.front_end.torchvision_loader.create_specs import TorchVisionSpecCreator
from cuc.pipeline import ModelFactory
...
The Pipeline provides a comprehensive verification validation framework (`validate_verifier.py`) to ensure the soundness and correctness of the verification backend. The validation framework implements two levels of validation:

### CBR: Counterexample/Soundness Validation
...
3. Run abstract analysis through the framework → get abstract bounds for each layer
...
python -m cuc.pipeline --validate-verifier --device cpu --dtype float64

# Run only counterexample validation (CBR)
python -m cuc.pipeline --validate-verifier --mode counterexample

# Run only bounds validation (Level 3)
python -m cuc.pipeline --validate-verifier --mode bounds --samples 20

# Test specific networks
python -m cuc.pipeline --validate-verifier --networks mnist_mlp_small,mnist_cnn_small

# Test with specific solvers
python -m cuc.pipeline --validate-verifier --mode counterexample --solvers gurobi torchlp

# Test with different transfer function modes
python -m cuc.pipeline --validate-verifier --mode bounds --tf-modes interval hybridz
```
...
# Run CBR: Counterexample validation
...
- name: Run Verifier Validation (float64)
  run: python -m cuc.pipeline --validate-verifier --device cpu --dtype float64

# Float32 test suite  
- name: Run Verifier Validation (float32)
  run: python -m cuc.pipeline --validate-verifier --device cpu --dtype float32
```

**Exit Codes:**
- `0`: All validations passed (no soundness bugs)
- `1`: Soundness bugs detected (CBR) or unsound bounds (Level 3)

**Test Networks:**

The validator uses test networks from `cuc/back_end/examples/nets/`:
...
## Integration with the framework Framework

The pipeline seamlessly integrates with the framework's core components:

- **Back-End Integration**: Direct use of `cuc.back_end` verification components
- **Front-End Bridge**: Integration with `cuc.front_end` spec creators and synthesis
- **Device Management**: Proper CUDA/CPU device handling
- **Configuration Compatibility**: Works with existing the framework configuration systems

Example integration:

```python
from cuc.front_end.torchvision_loader.create_specs import TorchVisionSpecCreator
from cuc.front_end.model_synthesis import synthesize_models_from_specs
...
This design provides a robust, extensible testing framework that ensures the reliability and performance of the abstraction verifier while being easy to use and extend.

### Back-End Integration

Convert to the framework and verify:

```python
from cuc.pipeline import TorchToCUC
from cuc.back_end.verifier import verify_once
...
- PyTorch models are exactly equivalent to Nets
- Weight transfer ensures bit-perfect copying
- Validated with maximum difference of 0.00e+00

### 3. Optional Dependencies
- Core functionality (ModelFactory, TorchToCUC) has minimal dependencies
...
```python
from cuc.pipeline import MockInputFactory, PipelineValidator
factory = MockInputFactory()
...
**CLI Usage:**
```bash
python cuc/pipeline/cli.py --strict-mode [options...]
```

**Programmatic Usage:**
```python
from cuc.front_end.verifiable_model import VerifiableModel
...
- [ ] the framework → PyTorch conversion (inverse direction)
