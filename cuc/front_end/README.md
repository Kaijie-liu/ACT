# Front End: Specification Creators & Unified CLI

The the framework front-end provides two specification creators for generating verification tasks from different data sources, with a unified CLI interface featuring automatic detection.
...
python -m cuc.front_end --list

# List only TorchVision datasets
python -m cuc.front_end --list --creator torchvision

# List only VNNLIB categories
python -m cuc.front_end --list --creator vnnlib

# Show available creators with details
python -m cuc.front_end --list-creators
...
python -m cuc.front_end --search mnist         # Finds: MNIST, FashionMNIST, KMNIST, EMNIST
python -m cuc.front_end --search cifar         # Finds: CIFAR10, CIFAR100, cifar100_2024
python -m cuc.front_end --search imagenet      # Finds: ImageNet (TorchVision)
python -m cuc.front_end --search yolo          # Finds: yolo_2023 (VNNLIB)
python -m cuc.front_end --search transformer   # Finds: vit_2023, safenlp_2024 (VNNLIB)
python -m cuc.front_end --search acas          # Finds: acasxu_2023 (VNNLIB)

# Search with creator filter
python -m cuc.front_end --search mnist --creator torchvision
python -m cuc.front_end --search cifar --creator vnnlib
...
python -m cuc.front_end --info MNIST           # Dataset info + recommended models
python -m cuc.front_end --info CIFAR10         # Shows: resnet18, mobilenet_v2, etc.
python -m cuc.front_end --info ImageNet        # Large-scale dataset info
python -m cuc.front_end --info FashionMNIST    # Fashion items dataset

# Auto-detect and show info (VNNLIB categories)
python -m cuc.front_end --info acasxu_2023     # ACAS Xu collision avoidance
python -m cuc.front_end --info vit_2023        # Vision Transformer verification
python -m cuc.front_end --info yolo_2023       # YOLO object detection
python -m cuc.front_end --info cifar100_2024   # CIFAR100 VNNLIB benchmark

# Explicit creator override (when name could be ambiguous)
python -m cuc.front_end --info MNIST --creator torchvision
python -m cuc.front_end --info cifar100_2024 --creator vnnlib
...
python -m cuc.front_end --download MNIST              # → MNIST + simple_cnn, lenet5, resnet18, etc.
python -m cuc.front_end --download CIFAR10            # → CIFAR10 + resnet18, mobilenet_v2, etc.
python -m cuc.front_end --download FashionMNIST       # → FashionMNIST + models
python -m cuc.front_end --download ImageNet           # → ImageNet (large!)

# Auto-detect and download (VNNLIB - downloads ONNX models + VNNLIB properties)
python -m cuc.front_end --download acasxu_2023        # → 45 ONNX models + properties
python -m cuc.front_end --download vit_2023           # → Vision Transformer benchmarks
python -m cuc.front_end --download yolo_2023          # → YOLO verification benchmarks
python -m cuc.front_end --download cifar100_2024      # → CIFAR100 VNNLIB benchmarks

# Force specific creator (if name could match multiple)
python -m cuc.front_end --download MNIST --creator torchvision
python -m cuc.front_end --download cifar100_2024 --creator vnnlib
...
python -m cuc.front_end --list-downloads

# List only TorchVision downloads
python -m cuc.front_end --list-downloads --creator torchvision

# List only VNNLIB downloads
python -m cuc.front_end --list-downloads --creator vnnlib
...
python -m cuc.front_end --synthesis

# Run synthesis for specific creator
python -m cuc.front_end --synthesis --creator torchvision   # PyTorch models with specs
python -m cuc.front_end --synthesis --creator vnnlib        # ONNX models with VNNLIB specs

# Run inference on synthesized models (defaults to TorchVision)
python -m cuc.front_end --inference

# Run inference for specific creator
python -m cuc.front_end --inference --creator torchvision   # Validates PyTorch models
python -m cuc.front_end --inference --creator vnnlib        # Validates ONNX→PyTorch models
...
python -m cuc.front_end --download MNIST && \
python -m cuc.front_end --download CIFAR10 && \
python -m cuc.front_end --download acasxu_2023

# Search and download pipeline
python -m cuc.front_end --search acas          # Find available ACAS benchmarks
python -m cuc.front_end --info acasxu_2023     # Check details
python -m cuc.front_end --download acasxu_2023 # Download it

# Complete TorchVision workflow
python -m cuc.front_end --download MNIST       # Download dataset + models
python -m cuc.front_end --synthesis            # Generate wrapped models
python -m cuc.front_end --inference            # Validate correctness

# Check what's available vs what's downloaded
python -m cuc.front_end --list                 # Show all available
python -m cuc.front_end --list-downloads       # Show what's downloaded
```
...
python -m cuc.front_end.cli --download MNIST
...
python -m cuc.front_end --download mnist --creator vnnlib
python -m cuc.front_end --list --creator torchvision
```
...
python -m cuc.front_end.torchvision_loader --models-for CIFAR10
python -m cuc.front_end.torchvision_loader --datasets-for resnet18
python -m cuc.front_end.torchvision_loader --validate MNIST resnet18
...
python -m cuc.front_end.torchvision_loader --download MNIST simple_cnn
```

### VNNLIB CLI (`vnnlib/cli.py`)
```bash
# VNNLIB-specific features
python -m cuc.front_end.vnnlib_loader --list
python -m cuc.front_end.vnnlib_loader --info acasxu_2023
python -m cuc.front_end.vnnlib_loader --download cifar100_2024 --max 10
python -m cuc.front_end.vnnlib_loader --parse-vnnlib path/to/file.vnnlib
```
...
```python
from cuc.front_end.creator_registry import detect_creator, get_creator
...
```
front_end/
├── __main__.py                  # 🆕 Entry point: python -m cuc.front_end
...
│   ├── __main__.py              # Entry point: python -m cuc.front_end.torchvision_loader
│   ├── README.md
...
└── vnnlib/                      # VNNLIB Creator  
    ├── __main__.py              # 🆕 Entry point: python -m cuc.front_end.vnnlib_loader
    ├── README.md                # 🆕
    ├── cli.py                   # 🆕 Domain-specific CLI
    ├── create_specs.py
...
1. **Spec Creation** → 2. **Model Synthesis** → 3. **Torch→the framework** → 4. **Verification**
...
from cuc.front_end.model_synthesis import model_synthesis
wrapped_models, input_data = model_synthesis(spec_results=results)

# 3. Convert to Net
from cuc.pipeline.verification.torch2cuc import torch_to_act_net
...
# 🧩 Spec-Free, Input-Free Torch→the framework + Verification — Two-File Design

This document specifies a compact, production-ready pattern to convert **wrapped PyTorch models** to the framework
and run verification with **no external inputs or specs** passed at runtime.

**Exactly two files to implement**:
- `../pipeline/torch2cuc.py` — Torch→the framework converter (reads embedded specs from wrapper; no input_shape needed)
- `verifier.py` — Single-shot (**verify_once**) and Branch-and-Bound (**verify_bab**) verification using only the `Net`
...
## 📦 File 1: `../pipeline/torch2cuc.py` (Converter)

### Responsibilities
- Convert the wrapper into a `Net` of `Layer` objects.
...
- Enforce a verification-ready wrapper via **hard assertions**:
  ...
  - last module is `OutputSpecLayer` (the framework last layer is `ASSERT`).

### Module → Layer Mapping
| Torch module | the framework kind | Notes |
|--------------|----------|-------|
...
This module is **spec-free, input-free**. All constraints are extracted from the `Net`.
...
import numpy as np
import torch
from cuc.back_end.solver.solver_base import Solver
...
- **Unsupported Torch modules** should raise `NotImplementedError` in `../pipeline/torch2cuc.py`.
...
- [ ] Implement `../pipeline/torch2cuc.py` with strong assertions and mapping table.
