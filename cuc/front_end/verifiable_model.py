...
# constraint checking during inference and seamless conversion.
...
from cuc.back_end.layer_schema import LayerKind, REGISTRY
...
class InputLayer(nn.Module):
    ...
    def to_act_layers(self, layer_id_start: int, in_vars: List[int]) -> Tuple[List, List[int]]:
        """Convert to Layer(s) and return (layers, out_vars)"""
        N = prod(self.shape[1:])
        out_vars = list(range(len(in_vars), len(in_vars) + N))
        
        # Collect params (optional: labeled_input for serialization)
        params = {
            "labeled_input": self.labeled_input,
        }
...
class InputSpecLayer(nn.Module):
    """
    Wraps the framework's InputSpec AND is an nn.Module. Returns constraint checking tuple.
    ...
    def to_act_layers(self, layer_id_start: int, in_vars: List[int]) -> Tuple[List, List[int]]:
        """Convert to Layer(s) - INPUT_SPEC doesn't create new vars"""
        params = {}
        for name in ("lb", "ub", "center", "A", "b"):
            val = getattr(self, name, None)
            if val is not None:
...
class OutputSpecLayer(nn.Module):
    """
    Wraps the framework's OutputSpec AND is an nn.Module. Returns constraint checking tuple.
    ...
    def to_act_layers(self, layer_id_start: int, in_vars: List[int]) -> Tuple[List, List[int]]:
        """Convert to Layer(s) - ASSERT doesn't create new vars"""
        params = {}
        for name in ("c", "lb", "ub"):
            val = getattr(self, name, None)
            if val is not None:
