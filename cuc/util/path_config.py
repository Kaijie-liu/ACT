# Python path configuration utilities, ensuring proper module imports
# and path resolution across the verification framework components.

import os
import sys
from typing import Any, Optional, Tuple

def setup_cuc_paths() -> str:
    """Set up project paths for proper module imports."""
    current_file = os.path.abspath(__file__)
    # From: /path/to/cuc/util/path_config.py
    # Need to go up 2 levels: util -> act
    cuc_root = os.path.dirname(os.path.dirname(current_file))
    
    # Add both cuc_root and its parent (project_root) to sys.path
    ...
    project_root = os.path.dirname(cuc_root)
...
def get_project_root() -> str:
    """Get the project root directory (parent of cuc/)."""
    current_file = os.path.abspath(__file__)
...
def ensure_gurobi_license() -> Optional[str]:
    ...
    if existing_license:
        ...
        print(f"[INFO] Using existing Gurobi license: {license_path}")
        return license_path

    if 'CUCHOME' in os.environ:
        acthome = os.environ['CUCHOME']
        print(f"[INFO] Using CUCHOME environment variable: {acthome}")
        license_path = os.path.abspath(os.path.join(acthome, 'modules', 'gurobi', 'gurobi.lic'))
        if os.path.exists(license_path):
            os.environ['GRB_LICENSE_FILE'] = license_path
            print(f"[INFO] Gurobi license found and set: {license_path}")
            return license_path
        else:
    ...
    print(f"[INFO] Auto-detecting project root from path_config")
...
def get_examples_gen_config_path() -> str:
    ...
    return os.path.join(get_project_root(), 'act', 'back_end', 'examples', 'config_gen_cuc_net.yaml')
...
def get_pipeline_log_dir() -> str:
    ...
    Returns:
        Absolute path to cuc/pipeline/log/ directory
    """
    return os.path.join(cuc_root, 'pipeline', 'log')
