# Core data structures for verification framework including Layer,
...
from cuc.back_end.layer_util import validate_layer, validate_graph, validate_wrapper_graph

# Supported layer types: Please see them in cuc/back_end/layer_schema.py
@dataclass
class Layer:
...
class ConSet:
    ...
    def add_op(self, tag: str, var_ids: List[int], **meta):
        ...
        if op and not self._is_op_supported_by_exporter(op):
            raise ValueError(
                ...
                "Update cuc/back_end/layer_schema.py SUPPORTED_EXPORT_OPS "
    ...
    def _is_op_supported_by_exporter(op: str) -> bool:
        ...
        """
        try:
            mod = importlib.import_module("cuc.back_end.layer_util")
