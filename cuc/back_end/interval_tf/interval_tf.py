...
from cuc.back_end.core import Bounds, Fact, Layer, Net, ConSet
from cuc.back_end.transfer_functions import TransferFunction
from cuc.back_end.layer_schema import LayerKind
from cuc.back_end.interval_tf.tf_mlp import *
from cuc.back_end.interval_tf.tf_cnn import *
from cuc.back_end.interval_tf.tf_rnn import *
from cuc.back_end.interval_tf.tf_transformer import *
...
class IntervalTF(TransferFunction):
    ...
    _LAYER_REGISTRY = {
        ...
        "MUL": lambda L, bounds, tf: tf_mul(L,
            tf._net.get_predecessor_bounds(L.id, tf._after, tf._before, 0),
            tf._net.get_predecessor_bounds(L.id, tf._after, tf._before, 1)),
        "DIV": lambda L, bounds, tf: tf_div(L,
            tf._net.get_predecessor_bounds(L.id, tf._after, tf._before, 0),
            tf._net.get_predecessor_bounds(L.id, tf._after, tf._before, 1)),
        "POW": lambda L, bounds, tf: tf_pow(L,
            tf._net.get_predecessor_bounds(L.id, tf._after, tf._before, 0),
            tf._net.get_predecessor_bounds(L.id, tf._after, tf._before, 1)),
        "MATMUL": lambda L, bounds, tf: tf_matmul(L,
            tf._net.get_predecessor_bounds(L.id, tf._after, tf._before, 0),
            tf._net.get_predecessor_bounds(L.id, tf._after, tf._before, 1)),
        ...
        "MAXPOOL1D": lambda L, bounds, tf: tf_maxpool1d(L, bounds),
        "MAXPOOL2D": lambda L, bounds, tf: tf_maxpool2d(L, bounds),
        "MAXPOOL3D": lambda L, bounds, tf: tf_maxpool3d(L, bounds),
        "AVGPOOL1D": lambda L, bounds, tf: tf_avgpool1d(L, bounds),
        "AVGPOOL2D": lambda L, bounds, tf: tf_avgpool2d(L, bounds),
        "PAD": lambda L, bounds, tf: tf_pad(L, bounds),
        "UPSAMPLE": lambda L, bounds, tf: tf_upsample(L, bounds),
        ...
        "GATHER": lambda L, bounds, tf: tf_gather(L, bounds),
        "INDEX_SELECT": lambda L, bounds, tf: tf_index_select(L, bounds),
