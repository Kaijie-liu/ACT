...
All sampling behavior is controlled by `config_gen_cuc_net.yaml`:
...
See `cuc/back_end/examples/config_gen_cuc_net.yaml` for details.

## Testing

Recommended checks:
- `python -m cuc.back_end --generate --config cuc/back_end/examples/config_gen_cuc_net.yaml --num 1`
- `python -m cuc.back_end --test-serialization --device cpu --dtype float32`

## Validation Strategy (Three-Layer Design)

NetFactory leverages the framework's existing validation infrastructure instead of implementing redundant checks. The validation responsibility is clearly separated across three stages:
...
**Where**: `cuc/back_end/layer_util.py` + `layer_schema.py`
...
**Where**: `cuc/back_end/serialization/serialization.py`

```python
from cuc.back_end.serialization import validate_json_schema
...
**Where**: `cuc/back_end/utils.py` (called by verifier)
...
1. **No Redundancy**: Reuses authoritative validation logic written by the framework core team
...
The `schema` section in `config_gen_cuc_net.yaml` serves as **documentation** to explain network classes and constraints, not as executable validation logic.
...
The schema is defined in `config_gen_cuc_net.yaml` under the `schema` section:
...
See `schema` section in `config_gen_cuc_net.yaml` for complete specifications including:
