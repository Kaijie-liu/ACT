...
```bash
# Generate networks from config_gen_cuc_net.yaml
python cuc/back_end/net_factory.py
...
- `config_gen_cuc_net.yaml` - Generator configuration for random networks
- `nets/*.json` - Generated network files (saved in nets/ subfolder)

## Architecture

The factory is extremely simple:
1. Reads generator configuration from `config_gen_cuc_net.yaml`
2. Samples network specs and constructs Nets in memory
