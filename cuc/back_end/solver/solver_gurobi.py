...
import numpy as np
import os
from cuc.back_end.solver.solver_base import Solver, SolverCaps, SolveStatus
from cuc.util.path_config import get_project_root
...
def setup_gurobi_license():
    ...
    if 'GRB_LICENSE_FILE' not in os.environ:
        if 'CUCHOME' in os.environ:
            license_path = os.path.join(os.environ['CUCHOME'], 'modules', 'gurobi', 'gurobi.lic')
            print(f"[INFO] Using CUCHOME environment variable: {os.path.relpath(os.environ['CUCHOME'])}")
        else:
            project_root = get_project_root()
            license_path = os.path.join(project_root, 'modules', 'gurobi', 'gurobi.lic')
            print(f"[INFO] Auto-detecting project root: {os.path.relpath(project_root)}")
        
        license_path = os.path.abspath(license_path)
        
        if os.path.exists(license_path):
            os.environ['GRB_LICENSE_FILE'] = license_path
            print(f"[INFO] Gurobi license found: {os.path.relpath(license_path)}")
        else:
        ...
        print(f"[INFO] Using existing Gurobi license: {os.path.relpath(os.environ['GRB_LICENSE_FILE'])}")
