# Layer utility functions for layers and networks. Separated from
...
try:
    ...
    from cuc.back_end.layer_schema import REGISTRY, LayerKind, SUPPORTED_EXPORT_OPS
    from cuc.front_end.specs import InKind, OutKind
...
def validate_layer(layer: "Layer") -> None:
    ...
    if kind == LayerKind.INPUT.value and 'dtype' in layer.meta:
        from cuc.util.device_manager import get_default_dtype
...
def create_layer(id: int, kind: str, params: Dict[str, Any], meta: Dict[str, Any],
    ...
    try:
        ...
        project_root = get_project_root()
        sys.path.insert(0, project_root)
        from cuc.back_end.core import Layer
