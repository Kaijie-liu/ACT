# JSON serialization and deserialization for Net and Layer structures.
...
from cuc.back_end.core import Layer, Net
from cuc.back_end.layer_schema import REGISTRY, LayerKind
from cuc.back_end.layer_util import validate_layer

# Version for format compatibility
SERIALIZATION_VERSION = "1.0"

class CUCSerializationError(Exception):
...
class TensorEncoder:
    ...
    def encode_tensor(tensor: torch.Tensor) -> Dict[str, Any]:
        ...
        if not HAS_TORCH:
            raise CUCSerializationError("PyTorch not available for tensor encoding")
    ...
    def decode_tensor(tensor_dict: Dict[str, Any], target_device: Optional[str] = None) -> torch.Tensor:
        ...
        if not HAS_TORCH:
            raise CUCSerializationError("PyTorch not available for tensor decoding")
...
class NetSerializer:
    ...
    def serialize_net(net: Net, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        net_metadata = {
            ...
            "framework_version": f"{SERIALIZATION_VERSION}",
...
class CUCJSONEncoder(json.JSONEncoder):
    """Custom JSON encoder for framework objects."""
    
    def default(self, obj):
        if isinstance(obj, (Layer, Net)):
            raise CUCSerializationError("Use NetSerializer.serialize_net() instead of json.dumps() directly")
...
def save_net_to_string(net: Net, metadata: Optional[Dict[str, Any]] = None, 
                       indent: int = 2) -> str:
    """Serialize Net to JSON string."""
    net_dict = NetSerializer.serialize_net(net, metadata)
    return json.dumps(net_dict, indent=indent, ensure_ascii=False, cls=CUCJSONEncoder)

def load_net_from_string(json_str: str, target_device: Optional[str] = None) -> Tuple[Net, Dict[str, Any]]:
    """Deserialize Net from JSON string with metadata."""
