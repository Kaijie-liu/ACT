...
# Assumes Net already encodes input and output specifications via
# INPUT_SPEC and ASSERT layers produced by torch2cuc.TorchToCUC.

# Public API:
#   - verify_once(net, solver, timelimit=None) -> VerifyResult
#
# Notes:
#   * Spec-free verification: all constraints extracted from Net layers.
...
# backend imports
from cuc.back_end.core import Bounds, Con, ConSet, Fact
from cuc.back_end.solver.solver_base import Solver, SolveStatus
from cuc.back_end.layer_schema import LayerKind
from cuc.back_end.utils import validate_constraints

# Front-end enums (kinds)
from cuc.front_end.specs import InKind, OutKind

# Verification types (canonical location: cuc/util/stats.py)
from cuc.util.stats import VerifyStatus, VerifyResult
...
def add_negated_assert_to_solver(solver: Solver, out_ids: List[int], assert_layer) -> None:
    ...
    from cuc.back_end.cons_exportor import to_numpy
...
def setup_and_solve(
    ...
    """
    from cuc.back_end.analyze import analyze
    from cuc.back_end.cons_exportor import export_to_solver
