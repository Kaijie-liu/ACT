...
from fidelity_supervised.flow import supervise,audit,costs
from sparse_basis.engine import POLICY as COMPONENT_POLICY
from fidelity_supervised.native import OPTIONS as NATIVE_OPTIONS, preflight, snapshot_input
from sparse_basis.engine import Budget

FREEZE=ROOT/'docs/fidelity_supervised_real_v2_freeze.json'
REVIEW=ROOT/'docs/fidelity_supervised_real_v2_selection_review.json'
CONTROL_REVIEW=ROOT/'docs/fidelity_supervised_v2_review.json'
OUTPUT=ROOT/'data/moe/results/fidelity_supervised_real_20260920_v2'
...
POLICY={'requests':4,'indices':[220,222,230,232],
    ...
    'construction':'unchanged sparse original-coordinate basis; fidelity import V2, no retries',
    'native_options':dict(NATIVE_OPTIONS),
    'import_contract':'intended binary64 model readback must match exactly; no warning bypass/scaling/pruning',
...
def verify_old():
    from fidelity_supervised.controls import old
    old()


def hashes():
    from fidelity_supervised.flow import sources
    return sources()
...
def compatibility(jobs):
    ...
    for j in jobs:
        ...
        b=Budget(time.monotonic()+300)
        imported=preflight(snapshot_input(v['lp'],b),b.tick)
        result.append({'job_id':j['job_id'],'variables':n,'E_rows':ne,'A_rows':na,
                       ...
                       'import_preflight':imported,'native_basis_mapping':'UNMEASURED',
                       'real_native_readback':'UNMEASURED','fill_bits_rank_runtime':'UNMEASURED'})
    return result
...
def runtime():
    ...
    from fidelity_supervised.native import VERSION
...
def freeze(controls):
    ...
    jobs=select();v={'schema':'FROZEN_FIDELITY_BASIS_DIAGNOSTIC_V2','status':'FROZEN_NOT_EXECUTED','policy':POLICY,
