...
from sparse_basis.engine import POLICY
...
def sources():
    paths = [p for folder in ('fidelity_supervised', 'sparse_basis', 'exact_primal')
...
def verify_sources(plan):
    if (plan['sources'] != sources() or plan['schema'] != 'FIDELITY_BASIS_SUPERVISION_PLAN_V2' or
            plan['component_policy'] != POLICY or plan['native_options'] != NATIVE_OPTIONS or
    ...
    try:
        ...
        for name in STAGES:
            if name == 'check':
                ...
                cmd = [ACT, '-m', 'fidelity_supervised.worker', name, str(root)]
...
def review_candidate(root, *, deadline=None):
    ...
    for i, row in enumerate(c['stages']):
        ...
        if row['state'] == 'LIMIT':
            ...
            if bound['phase'] != name or bound['deadline_monotonic'] != p['started'] + limit or bound['policy'] != POLICY:
    ...
    if 'capture' in names:
        ...
        if (r['schema'] != 'FIDELITY_NATIVE_BASIS_CAPTURE_V2' or r['policy'] != POLICY or
    ...
    if 'construct' in names:
        ...
        if (construction['schema'] != 'SPARSE_BASIS_PROPOSAL_V1' or construction['policy'] != POLICY or
...
def supervise(spec, destination, *, started):
    ...
    save_new(root / 'plan.json', {'schema': 'FIDELITY_BASIS_SUPERVISION_PLAN_V2', 'spec': spec, 'started': started,
                                 'total_seconds': 300, 'work_seconds': 298, 'proposal_seconds': 218,
                                 'component_policy': dict(POLICY), 'native_options':dict(NATIVE_OPTIONS), 'sources': sources()})
    ...
    try:
        ...
        with (root / 'driver.log').open('xb') as log:
            proc = subprocess.Popen([ACT, '-m', 'fidelity_supervised.flow', str(root)], cwd=ROOT,
    ...
    if observed >= 300:
        status = 'TIMEOUT'
    terminal = {'schema': 'SUPERVISED_FIDELITY_BASIS_V2', 'status': status,
...
def audit(root):
    ...
    verify_sources(p)
    if v['schema'] != 'SUPERVISED_FIDELITY_BASIS_V2':
