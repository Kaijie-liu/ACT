...
from fidelity_supervised.flow import (supervise, audit, costs, review_candidate, sources,
                                    owned_stage, inventory, semantic_stop, phase_state)
from fidelity_supervised.batch import loop, summarize
...
class Controls(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        ARTIFACT_ROOT = Path(tempfile.mkdtemp(prefix='fidelity_supervision_controls_', dir=ROOT / 'data/moe/results'))
    ...
    def test_native_boundary_cutoff_preserves_partial(self):
        ...
        row = owned_stage(root, 'capture', [ACT, '-m', 'fidelity_supervised.fault_worker', 'native_stall', str(root)], start, 218)
    ...
    def test_exact_elimination_cutoff_preserves_partial(self):
        ...
        row = owned_stage(root, 'construct', [ACT, '-m', 'fidelity_supervised.fault_worker', 'construct_stall', str(root)], start, 218)
    ...
    def test_postcapture_exception_preserves_raw(self):
        ...
        row = owned_stage(root, 'map', [ACT, '-m', 'fidelity_supervised.fault_worker', 'map_exception', str(root)], start, 218)
    ...
    def test_full_outer_faults_terminal_and_costs(self):
        ...
        for mode, phase, elapsed, expected in (
            ...
            def launch(cmd, *args, **kwargs):
                self.assertEqual(cmd[1:3], ['-m', 'fidelity_supervised.flow'])
                return original_popen([ACT, '-m', 'fidelity_supervised.fault_worker', 'drive_' + mode, str(root)], *args, **kwargs)
            with patch('fidelity_supervised.flow.subprocess.Popen', launch):
    ...
    def test_checker_deadline_partial_output_not_accepted(self):
        ...
        row = owned_stage(root, 'check', [ACT, '-m', 'fidelity_supervised.fault_worker', 'check_stall', str(root)], start, 298)
    ...
    def test_export_decoder_all_bindings(self):
        from fidelity_supervised.inputs import load
    ...
    def test_raw_only_cost_binding_rejects_wrong_request(self):
        from fidelity_supervised.flow import raw_cost
    ...
    def test_new_plan_policy_and_old_version_rejected(self):
        from fidelity_supervised.flow import verify_sources
    ...
    def test_study_ordered_error_roster(self):
        from fidelity_supervised.study import loop as study_loop
