...
## 本轮执行结果（前向控制冻结提交5b5da4fa2）

[紧凑归档](dual_rs_author_install_20260921.json)保存8条安装/获取/入口/前向回执，
独立文件重读与环境清单比对通过。作者checkout保持干净，五个原环境及MetaMoE的
R1/R2环境清单均不变。未中断其他GPU任务，探针结束后显存恢复到原24,231MiB占用。

| 阶段 | 实际结果 | 墙钟 |
|---|---|---:|
| 新venv／依赖 | 完成，pip check通过 | 1.42s／65.04s |
| 两个公开模型及配置 | 完整取得，ViT LFS校验通过 | 546.56s |
| 三个作者CLI | 全部加载通过 | 各约1.91–2.31s，另列postflight |
| 原生预训练denoiser＋ViT | index0，三种sigma均返回有限10维输出 | 全控制4.010s |

实际控制输入label3，三个随机噪声前向都预测3；这不是统计准确率或认证率。
作者噪声映射得到timestep1158/1984/2809，保留原可变噪声路径，未冻结成常权重分类器。
峰值allocated876,519,424字节（约0.816GiB），reserved897,581,056字节。
首次与后续前向受启动影响，不用单样本时间推算正式认证或90epoch训练ETA。

下载身份、调用、输出和成本均归档；生成与检查未接入ACT旧主表。
26项控制测试通过（含下载哈希错误保留partial、拒绝覆盖），旧论文表格重建未变。
仍没有trained sigma-estimator，因此本轮**没有**fresh Dual RS认证或完整方法比较。

预训练下一步的代码核查：作者90epoch循环每epoch覆盖`checkpoint.pth.tar`，
即最终epoch规则，不是按测试表现选best；`--resume`虽定义，但未执行恢复，
保存内容也没有scheduler状态。后续长训练监督不能假设该flag提供可靠断点续训。
先检查一步训练、输出/标签绑定及恢复语义，再单独冻结；本轮未启动长训练。
