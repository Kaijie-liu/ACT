...
| MetaMoE |原生PyTorch作者后端组件均正；完整20类R2 smoke两臂复放CIFAR0误分类、作者MNIST0正；R3全router稀疏传播已通过|尚未复跑全部router/expert表；旧ONNX失败保留|R3 guarded专家0的下一卷积被2GiB保守准入门拒绝（非OOM）；完整smoke与新20输入门仍关闭。不是已完成正式比较|
