...
| MetaMoE |原生PyTorch作者后端组件均正；完整20类R2 smoke两臂复放CIFAR0误分类、作者MNIST0正；R4旧输入完整router＋两个guarded专家表示通过|尚未复跑全部router/expert表；旧ONNX失败保留|新表示门已通过（8.51s、零求解、不提高内存门）；仍须新版本完整旧输入smoke，尚未执行新20输入正式比较|
