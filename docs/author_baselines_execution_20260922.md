...
| MetaMoE |R4完整旧输入四调用及独立审计/原模型复放完成；两臂CIFAR0同中心UNSAFE，作者MNIST0数值正过滤，ACT MNIST0输出UNKNOWN|尚未复跑全部router/expert表；旧ONNX及容量失败保留|表示及完整执行门通过，非ACT输出认证成功；新20输入正式比较未选择/未执行|
...
最新[完整R4 smoke结果](metamoe_csr_smoke_result_20260922_r4.md)已执行并归档：
4/4外层正常完成、无超时/资源拒绝，整批221.024s。CIFAR0两臂复放同一个既有
中心误分类；MNIST作者数值正过滤7.115s，ACT UNKNOWN209.488s。ACT路由覆盖已完成、
selected-score非零范围被接受，但expert1输出性质未闭合；不归因成模型不安全、
不自动称为松弛精度失败。完整smoke执行门及独立复放通过，不是ACT严格证书。
485身份绑定保持、未新选样本、未启动正式20输入。下列“仅冻结/未执行”是历史阶段。

最新[R4旧输入控制](metamoe_csr_result_20260922_r4.md)已通过：卷积因子支持并集
...
专家0后，layer7卷积预计总收费3.101GiB超过冻结2GiB，构造前拒绝；实际峰值采样RSS
1.495GiB、7.426s、零求解。**不是实测OOM，也不是负安全下界。** 新正式门仍关闭，
未执行R3完整smoke或选择20新输入。下面保留R2阻塞的历史来由，不再把“首router
ReLU未过”作为R3当前状态。当前机器台账仍是上轮冻结快照；本结果单独绑定归档。
