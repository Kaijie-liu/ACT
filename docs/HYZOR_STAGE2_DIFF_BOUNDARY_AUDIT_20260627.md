...
`60 files changed, 23788 insertions(+), 639 deletions(-)`

Directory-level split:

| Area | Files | Added | Deleted | Status |
|---|---:|---:|---:|---|
| `act/pipeline` | 15 | 10771 | 141 | largest remaining consolidation target |
...
|---|---:|---:|---|
| `act/pipeline/hybridz_sparse_exact_probe.py` | 4772 | 0 | biggest remaining package-level prototype; first post-freeze consolidation target is partially reduced |
...
0. First consolidation passes completed after this audit:
   ...
   product worker.  The local duplicate probe code dropped from 5751 to 4772
   lines
