...
`60 files changed, 24039 insertions(+), 639 deletions(-)`

Directory-level split:

| Area | Files | Added | Deleted | Status |
|---|---:|---:|---:|---|
| `act/pipeline` | 15 | 11049 | 141 | largest remaining consolidation target |
| `act/back_end/hybridz_tf` | 8 | 4782 | 292 | core HZ operator/product path |
| `docs` | 16 | 3786 | 0 | audit/provenance/future-work docs |
...
|---|---:|---:|---|
| `act/pipeline/hybridz_sparse_exact_probe.py` | 5050 | 0 | biggest remaining package-level prototype; first post-freeze consolidation target is partially reduced |
...
0. First consolidation passes completed after this audit:
   ...
   removed from the packaged probe, and max-layer partial propagation was
   removed from the packaged probe.  The local duplicate probe code dropped
   from 5751 to 5050 lines
