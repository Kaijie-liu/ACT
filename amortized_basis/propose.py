...
from amortized_basis.engine import POLICY, Budget, statistics, eliminate, Unresolved


def propose(lp, statement, candidate, hint, expected_statement, expected_hint, *, deadline, amortize_source=True, amortize_plan=True):
    ...
    try:
        ...
        vals = eliminate(system, b, stats, amortize_source=amortize_source,
    ...
    return {'schema': 'AMORTIZED_BASIS_PROPOSAL_V1', 'status': status, 'error': error,
            'bundle': bundle, 'row_residuals': residuals, 'stats': stats, 'operations': b.operations,
            'costs':b.costs(), 'amortize_source':amortize_source, 'amortize_plan':amortize_plan, 'policy': dict(POLICY), 'arithmetic': b.arithmetic, 'seconds': time.monotonic() - begin, 'deadline_monotonic': deadline,
