"""Immutable request-local validation amortization; controls only."""
...
from .immutable import ExactRational
...
POLICY = {**OLD_POLICY, 'arithmetic': 'bounded_multimodular_rational_reconstruction_V1',
          ...
          'live_accounting': 'source_active_pivots_plus_plan_and_owned_source_units',
          'plan_entries': 1000000, 'plan_reuse': 'checked_symbolic_schedule_owned_validation_V1'}
...
class Budget:
    ...
    def value(self,v):
        self.visit()
        if type(v) is ExactRational:v=F(v.numerator,v.denominator)
        v=v if isinstance(v,F) else rational(v)
...
def verify_residual(system, values, b):
    ...
    for row,rhs in system:
        ...
        for j,v in row.items():
            total = b.value(total+b.value(F(v.numerator,v.denominator)*values[j]))
        if total != F(rhs.numerator,rhs.denominator):
            return False
    return True


def eliminate(system, b, stats, *, amortize_source=True, amortize_plan=True, scope='analytic'):
    from .session import Session
    session=Session(system,b,stats,scope=scope,amortize_source=amortize_source,
                    amortize_plan=amortize_plan)
    ...
    for p in primes(b,stats):
        ...
        try:
            xmod = session.solve(p,scope=scope,source_id=session.source_id)
        except BadPrime:
