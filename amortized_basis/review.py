...
def review(receipt,output):
            ...
            if root==roots[0]:
                ...
                if ref.exists():
                    for name,v in read(ref).items():
                        ...
                        diffs+=1
    # Recompute the registered four-mode operation attribution from raw rows.
    modes={}
    for suffix in ('00','10','01','11'):
        modes[suffix]=read(roots[0]/('sparse1024_'+suffix)/'result.json')
    baseline=modes['00'];details={}
    for suffix,row in modes.items():
        if row['solution']!=baseline['solution']:raise ValueError('four-mode solution mismatch')
        ops=row['costs']['operations'];baseops=baseline['costs']['operations']
        delta=baseline['operations']-row['operations']
        explained=sum(baseops.get(k,0)-ops.get(k,0) for k in ('plan/binding','plan/validate'))
        if delta!=explained:raise ValueError('non-validation cost changed')
        st=row['stats'];expected_source=1 if suffix[0]=='1' else st['primes_tried']
        expected_plan=st['plan_builds']+(0 if suffix[1]=='1' else st['plan_replays'])
        if st['source_full_checks']!=expected_source or st['plan_full_checks']!=expected_plan:
            raise ValueError('full-check counts inconsistent with immutable amortization')
        if st['request_guards']!=st['primes_tried']+st['plan_builds']:
            raise ValueError('per-use guard omitted')
        details[suffix]={'operations':row['operations'],'seconds':row['seconds'],
                         'source_full_checks':st['source_full_checks'],
                         'plan_full_checks':st['plan_full_checks'],
                         'saved_operations':delta,'explained_validation_saving':explained,
                         'costs':row['costs'],'primes_used':st['primes_used']}
    base=Path(tempfile.mkdtemp(prefix='amortized_basis_review_',dir=ROOT/'data/moe/results'))
    checks=[]
    ...
    result={'status':'PASS','issues':[],'receipt':str(receipt),'receipt_sha256':digest(receipt.read_bytes()),
            ...
            'four_mode_attribution':details,'seconds':time.monotonic()-begin,'real_LP_calls':0,'new_eliminations':0,'native_calls':0,
