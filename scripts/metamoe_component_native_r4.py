"""R4: native PyTorch author backend entry, SAME two component requests.

Explicit export-bypass variant, not byte-identical author export.
Original specification/configuration/backend remain unchanged.
...
def worker(config, index, work):
    ...
    try:
        ...
        phase("data_loaded")
        onnx_path = work / "native_model_spec.json"
        spec_info = {"loader": str(Path(__file__).with_name("metamoe_native_model.py").resolve()),
                     "loader_sha256": sha256(Path(__file__).with_name("metamoe_native_model.py")),
                     "checkpoint": str(checkpoint), "checkpoint_sha256": item["checkpoint_sha256"],
                     "dtype": "float32", "export": "NONE_NATIVE_PYTORCH"}
        onnx_path.write_text(json.dumps(spec_info, indent=2) + "\n")
        direct = load_component(str(repo), str(checkpoint), item["checkpoint_sha256"])
        export_info = {"native_pytorch": True, "source_bn_count": sum(
            isinstance(m, torch.nn.modules.batchnorm._BatchNorm) for m in direct.modules())}
        errors = []
        for probe in [x, torch.zeros_like(x), torch.randn_like(x)]:
            with torch.no_grad():
                expected = model(probe)[0].numpy()
            with torch.no_grad():
                actual = direct(probe).numpy()
            error = float(np.max(np.abs(actual - expected)))
            if not math.isfinite(error) or error != 0.:
                raise ValueError(f"native wrapper removal not exact: {error}")
            errors.append(error)
        phase("native_pytorch_exact_probe_complete")
        ...
        backend["model"].pop("onnx_path", None)
        backend["model"]["name"] = "Customized(%r, 'load_component', %r, %r, %r)" % (
            spec_info["loader"], str(repo), str(checkpoint), item["checkpoint_sha256"])
        ...
        prepared = {"input_sha256": sha256(work / "input.npy"), "label": label,
                    ...
                    "native_model_spec_sha256": sha256(onnx_path), "vnnlib_sha256": sha256(spec),
                    ...
                    "guarantee_object": "original native PyTorch component plus decimal VNNLIB; real/backend policy, not floating execution proof"}
