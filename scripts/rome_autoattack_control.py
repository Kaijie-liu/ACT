"""One frozen raw-order input, native standard AutoAttack, full variable routes."""
import argparse
import hashlib
import json
import math
...
def validate(cfg):
    if (cfg['norm'], cfg['version'], cfg['batch_size'], cfg['source_s_b']) != ('Linf', 'standard', 1, [4, 6]):
        raise ValueError('unsupported frozen attack/source settings')
    if not math.isfinite(cfg['epsilon']) or not 0 < cfg['epsilon'] <= 1:
...
def worker(cfg, config):
    ...
    attacker = AutoAttack(wrapped, norm='Linf', eps=cfg['epsilon'], version='standard',
    ...
    write(root / 'result.json', {'status': 'ATTACK_EVALUATION_COMPLETED', 'config_sha256': sha256(config),
        ...
        'adversarial_prediction': int(output.argmax(1)), 'linf_distance': distance,
        ...
        'scope': 'single observed-input deployment control; not paper accuracy or ACT competitive table'})
