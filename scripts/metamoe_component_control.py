"""Bounded author-component deployment controls, not a full-MoE benchmark.

Runs unmodified author export/spec/config functions and the pinned author backend.
Only CPU/device, seed and output locations are made explicit in generated config.
"""
import argparse
...
def worker(config, index, work):
    ...
    try:
        ...
        phase("data_loaded")
        onnx_path = export_model_to_onnx(str(checkpoint), str(work / "onnx"),
                                         device="cpu", simplify_onnx=True, validate=False)
