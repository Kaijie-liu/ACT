...
if __name__ == '__main__':
    ...
    manifest = Path(f'configs/recent_moe/metamoe_paired_{suffix}_r1.json')
    tensors = Path(f'/data1/Kane/MOE/baseline_data/metamoe_paired_20260922_r1_{suffix}')
    tensors.mkdir(parents=True, exist_ok=False)
    if not a.smoke:
        audit = Path('docs/metamoe_paired_smoke_review_20260922_r1.json')
    ...
    for name in ['scripts/metamoe_paired_model.py', 'scripts/metamoe_paired_execution.py',
                 'scripts/freeze_metamoe_paired.py', 'scripts/audit_metamoe_paired.py',
                 'tests/test_metamoe_paired_model.py', 'configs/recent_moe/metamoe_component_control_r4.json',
                 'docs/metamoe_native_control_review_20260922_r4.json']:
        files[name] = sha256(name)
    cfg = {'protocol': f'metamoe_matched_full_request_{suffix}_r1', 'requests': requests,
           ...
           'output_root': f'/data1/Kane/MOE/baseline_runs/metamoe_paired_20260922_r1_{suffix}',
           ...
           'arms': ['ACT_class_separated_HZ_policy', 'author_native_backend_route_invariance_sufficient_adapter'],
