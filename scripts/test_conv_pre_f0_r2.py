...
from scripts.check_conv_pre_f0_r2 import aggregate,order_bounds,TRUSTED
...
def fixture(base=2):
    ...
    m={'schema':'CONV_PRE_F0_RATIONAL_R2_MANIFEST','request':req,'trusted_base':TRUSTED,'positive_threshold':1e-7,
...
class PreF0Tests(unittest.TestCase):
    ...
    def test_stdlib_only(self):
        code=('import json,sys;from scripts.check_conv_sign_lp import isolate;isolate();'
              'from scripts.check_conv_pre_f0_r2 import aggregate;a=json.load(sys.stdin);'
    ...
    def test_capture_stops_before_float_f0(self):
        from scripts import run_conv_pre_f0_r2 as run
        import torch
        ...
        with tempfile.TemporaryDirectory(dir=run.ROOT/'data/moe/results') as tmp,ExitStack() as stack:
            ...
            stack.enter_context(patch.object(mono,'linear_safety_rows',return_value=[
                (torch.tensor(property_vector(0,c),dtype=torch.float64),torch.tensor(0.,dtype=torch.float64)) for c in range(1,10)]))
