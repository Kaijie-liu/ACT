"""R3 short local IPC repair; SAME full-batch native PGD7 update/save.
...
def worker(manifest, path, arm):
    ...
    write(root/'prepared.json', {'config_sha256':sha256(path), 'arm':arm,
        'resolved_config':OmegaConf.to_container(cfg,resolve=False),
        'gpu':torch.cuda.get_device_name(0), 'initial_free_bytes':free,
        'local_ipc_root':ipc_root, 'memory_fraction_cap':.4, 'total_gpu_bytes':total})
