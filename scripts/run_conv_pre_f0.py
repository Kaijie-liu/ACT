...
from scripts.conv_pre_f0_contract import (ROOT,DEFAULT,FREEZE,read,save,sha,git,parents,job,sources,
...
from scripts.check_conv_pre_f0 import TRUSTED,order_bounds
...
def capture_worker(directory):
    ...
    def construct(*,model,center,clean_prediction,internal,config,reuse=None,budget=None):
        ...
        manifest.update(schema='CONV_PRE_F0_RATIONAL_R1_MANIFEST',request=expected_scope(j),trusted_base=TRUSTED,
...
def run():
    ...
    rt={'schema':'CONV_PRE_F0_RATIONAL_R1_RUN','execution_head':git('rev-parse','HEAD'),'freeze_sha256':sha(FREEZE),
    ...
    for name,cap in [('capture',300),('proposal',2100),('check',600),('independent',600)]:
        args=([sys.executable,'-m','scripts.run_conv_pre_f0','--'+name,str(directory)] if name in ('capture','proposal') else
              [sys.executable,'-S','-m','scripts.check_conv_pre_f0',str(directory),str(directory/(name+'.json'))])
...
if __name__=='__main__':
    ...
    if a.freeze:
        ...
        with (ROOT/'data/moe/results/conv_pre_f0_r1.lock').open('a') as lock:
