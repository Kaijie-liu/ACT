...
from basis_supervised.flow import (supervise, audit, costs, review_candidate, sources,
                                    owned_stage, inventory, semantic_stop, phase_state)
from basis_supervised.batch import loop, summarize
...
class Controls(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        ARTIFACT_ROOT = Path(tempfile.mkdtemp(prefix='basis_supervision_controls_', dir=ROOT / 'data/moe/results'))
    ...
    def test_actual_unsupported_mapping_stops_preserves_capture(self):
        ...
        root = self.base / 'unsupported'
        v = supervise(spec, root, started=time.monotonic())
        self.assertEqual(v['status'], 'UNSUPPORTED_MAPPING', v)
        self.assertEqual(audit(root)['status'], 'UNSUPPORTED_MAPPING')
        ...
        self.assertIsNone(costs(root)['phases']['construct']['seconds'])
        OBSERVATIONS.append({'case': 'unsupported_mapping', 'costs': costs(root), 'status': v['status']})
    ...
    def test_outer_owned_cutoff(self):
        ...
        v = supervise(self.spec, root, started=time.monotonic() - 297.98)
        self.assertEqual(v['status'], 'TIMEOUT')
    ...
    def test_native_boundary_cutoff_preserves_partial(self):
        ...
        row = owned_stage(root, 'capture', [ACT, '-m', 'basis_supervised.fault_worker', 'native_stall', str(root)], start, 218)
    ...
    def test_exact_elimination_cutoff_preserves_partial(self):
        ...
        row = owned_stage(root, 'construct', [ACT, '-m', 'basis_supervised.fault_worker', 'construct_stall', str(root)], start, 218)
    ...
    def test_postcapture_exception_preserves_raw(self):
        ...
        row = owned_stage(root, 'map', [ACT, '-m', 'basis_supervised.fault_worker', 'map_exception', str(root)], start, 218)
    ...
    def test_full_outer_faults_terminal_and_costs(self):
        ...
        for mode, phase, elapsed, expected in (
            ...
            def launch(cmd, *args, **kwargs):
                self.assertEqual(cmd[1:3], ['-m', 'basis_supervised.flow'])
                return original_popen([ACT, '-m', 'basis_supervised.fault_worker', 'drive_' + mode, str(root)], *args, **kwargs)
            with patch('basis_supervised.flow.subprocess.Popen', launch):
    ...
    def test_checker_deadline_partial_output_not_accepted(self):
        ...
        row = owned_stage(root, 'check', [ACT, '-m', 'basis_supervised.fault_worker', 'check_stall', str(root)], start, 298)
    ...
    def test_missing_or_tampered_evidence_rejected(self):
        for i, name in enumerate(('native/capture.json', 'construction.json', 'portable/bundle.json', 'check.log', 'map_stage.json')):
