"""Full supervised upstream reuse ablation; both arms use the same sole-check tail."""
import importlib
import math
...
def tail(arm):
    ...
    return importlib.import_module('single_check_portable.execution')
...
def validate_reuse_report(root, plan, candidate):
    from moe_evidence.schema import validate_request
    root=Path(root);report=read(root/'source/upstream_reuse_report.json')
    key='source/upstream_reuse_report.json'
    if candidate['artifact_sha256'].get(key)!=digest((root/key).read_bytes()):
        raise ValueError('missing/changed proposal report binding')
    enabled={'source_cache':plan['arm']=='both','matrix_cache':True}
    if (report['schema']!='UPSTREAM_REUSE_CONTROLS_V1' or report['error'] is not None or
            report['reason'] not in ('PROPOSAL_LOOP_RETURNED','RESERVE_EXHAUSTED_HANDOFF') or
            (report['total_seconds'],report['cap_seconds'],report['reserve_seconds'])!=(300,60,80) or
            report['scope']!=validate_request(plan['request']['evidence_request']) or
            report['next_stage']!='UNCHANGED_COMPLETE_CHECK' or report['not_a_proof_verdict'] is not True or
            any(report[k] is not False for k in ('check_elision','schedule_changed','acceptance_changed'))):
        raise ValueError('proposal policy or scope mismatch')
    for name in ('source_cache','matrix_cache'):
        cache=report[name]
        if cache['enabled'] is not enabled[name] or any(v!=0 for k,v in cache.items() if k.startswith('live_')):
            raise ValueError('cache arm/cleanup mismatch')
    from upstream_reuse.storage import LIMITS as SOURCE_LIMITS, POLICY as SOURCE_POLICY
    from exact_matrix_cache.cache import LIMITS as MATRIX_LIMITS, POLICY as MATRIX_POLICY
    for key,limits,policy in (('source_cache',SOURCE_LIMITS,SOURCE_POLICY),('matrix_cache',MATRIX_LIMITS,MATRIX_POLICY)):
        if report[key]['limits']!=limits or report[key]['policy']!=policy:raise ValueError('cache policy drift')
    row=candidate['stages'][1]
    if not row['start_seconds']<=report['elapsed_seconds']<=row['end_seconds']:
        raise ValueError('proposal clock reset/omitted setup')
    timers=report['timings']
    for value in timers.values():
        if (type(value['calls'])is not int or value['calls']<0 or
            not all(math.isfinite(value[k]) for k in ('inclusive_seconds','exclusive_seconds')) or
            not 0<=value['exclusive_seconds']<=value['inclusive_seconds']+1e-8):
            raise ValueError('invalid nested timing')
    # Root operations (initialization and loop) are disjoint; only exclusive
    # entries may be added. Their total fits the complete process window.
    if sum(v['exclusive_seconds'] for v in timers.values())>row['elapsed_seconds']+.01:
        raise ValueError('nested time double counted or clock reset')
    return report
...
def drive(root):
    ...
    try:
        ...
        for name in ('capture', 'propose'):
            ...
            with (root/(name+'.log')).open('xb') as log:
                proc = subprocess.Popen([ACT, '-m', 'source_cache_ablation.worker', name,
                       str(source), '--started', repr(started), '--arm', plan['arm']], cwd=ROOT, env=environment(),
    ...
    files += [source/'request.json', source/'manifest.json', source/'upstream_reuse_report.json',
              source/'capture.json', source/'query_log.json', source/'upstream_reuse_started.json', root/'tail_entered.json']
...
def review_candidate(root):
    ...
    if c['wall_seconds'] >= 298 and not (root/'tail/admission.json').exists():
        ...
        validate_reuse_report(root, plan, c)
        ...
        handoff = read(root/'source/upstream_reuse_report.json')
        if (handoff['total_seconds'], handoff['cap_seconds'], handoff['reserve_seconds']) != (300,60,80):
...
def supervise(request, arm, destination, *, started):
    ...
    try:
        ...
        with (root/'driver.log').open('xb') as log:
            proc = subprocess.Popen([ACT, '-m', 'source_cache_ablation.flow', str(root)],
    ...
    value = {'schema': 'SOURCE_CACHE_ATTRIBUTION_V1', 'status': status,
...
def costs(root):
    ...
    def saved(path):
        ...
        try:return read(path)
            ...
            unreadable.append(str(path.relative_to(root)));return None
    for name in ('capture','propose'):
        path=root/(name+'_stage.json');entered=root/(name+'_entered.json')
        row=saved(path);event=saved(entered)
        if row is not None:
            parts[name]={'seconds':row['elapsed_seconds'],'state':row['state'],
                                     'censored':row['state']=='TIMEOUT'}
        elif event is not None:
            parts[name]={'seconds':None,'observed_window_seconds':max(0,v['driver_exit_seconds']-event['seconds']),
                         'state':'INTERRUPTED','censored':True}
        elif path.exists() or entered.exists():parts[name]={'seconds':None,'state':'INTERRUPTED','censored':True}
    ...
    proposals=saved(query) if query.exists() else []
    ...
    windows=[p['seconds'] for p in parts.values() if p['seconds'] is not None]
    if sum(windows)>whole+.01:raise ValueError('phase costs exceed request clock')
    report=None
    if v.get('candidate_sha256') is not None:
        candidate=read(root/'candidate.json')
        if (len(candidate['stages'])==2 and candidate['stages'][1]['state']=='COMPLETED'
                and (root/'source/upstream_reuse_report.json').exists()):
            report=validate_reuse_report(root,read(root/'plan.json'),candidate)
    return {'whole_request_seconds':whole,
            'phases':parts,'proposal_queries':proposals,
            'proposal_metrics':report,
            'unreadable_interrupted_records':unreadable,
            'observed_phase_sum_seconds':sum(windows),
            'residual_clock_seconds':whole-sum(windows),
            'cost_identity':'whole = observed disjoint phase windows + residual; censored phases not zero',
            'measurement_endpoint':'outer publication observation; admission invalidated if its write reaches 300s',
