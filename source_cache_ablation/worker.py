...
def execute(stage, root, budget, arm):
    if arm not in ('matrix_only','both'): raise ValueError('unregistered arm')
    if stage == 'propose':
        ...
        run(root, request, budget, source_enabled=arm=='both', matrix_enabled=True)
    elif stage == 'capture':
...
if __name__ == '__main__':
    ...
    p.add_argument('--started',required=True,type=float);p.add_argument('--arm',required=True,choices=('matrix_only','both'))
    a=p.parse_args()
