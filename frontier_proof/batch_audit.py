...
def review(root):
    ...
    if [c['id'] for c in cfg['calls']] != ['exhaustive','checked_frontier']:
    ...
    if all((root/c['id']/'construction.json').exists() for c in cfg['calls']):
        old=load(root/'exhaustive/construction.json');new=load(root/'checked_frontier/construction.json')
    ...
    return {'audit':'INCOMPLETE' if issues else 'PASS','issues':len(issues),'issue_details':issues,
        ...
        'frontier_only_positive':'checked_frontier' in positives and 'exhaustive' not in positives,
        'exhaustive_only_positive':'exhaustive' in positives and 'checked_frontier' not in positives,
