...
CONFIG=ROOT/'configs/backend_controls/residual_proof_compare_r1.json'
RECORD=ROOT/'docs/residual_proof_freeze_review_20260925_r1.json'


def review():
    cfg=load(CONFIG);common=cfg['common'];scope=common['scope'];expected={}
    for name in ('shared_route_residual_synthetic_r1.json',):
    ...
    paths=[ROOT/'docs/residual_proof_protocol_20260925_r1.md',*sorted((ROOT/'residual_proof').glob('*.py'))]
    ...
    gate=load(ROOT/'docs/residual_proof_controls_20260925_r1.json',cfg['controls_sha256'])
    if gate['status']!='PASS' or gate['tests_passed']!=121 or gate['source_hashes']!=expected or gate['real_requests_executed']!=0:
    ...
    sample=next(s for s in selection['samples'] if s['sample_rank']==3);model=selection['models']['seed0']
    if sample['dataset_index']!=4099 or sample['label']!=4 or cfg['sample']!={k:sample[k] for k in ('sample_rank','dataset_index','label','center')}:
        raise ValueError('sequential fixed engineering object')
    want={'experts':8,'classes':10,'label':4,'radius':'2/255','clip':['0','1'],
    ...
    required=[{'pair':list(p),'label':4,'competitor':k} for p in combinations(range(8),2) for k in range(10) if k!=4]
    ...
    if (cfg['automatic_execution'] is not False or cfg['no_resume_retry_expansion'] is not True or cfg['calls_per_arm']!=1 or
        cfg['proposal_stage_fraction']!='1/2' or cfg['publication_reserve_seconds']!=2 or cfg['route_margin_count']!=56 or cfg['route_candidate_files']!={'pairwise':56,'shared':1} or
    ...
    calls=[{'id':m,'proof_policy':{'schema':'RESIDUAL_PROOF_POLICY_V1','mode':m,
        'router_proposal':'FINAL_AFFINE_DUAL_ONLY','parse_cache':False,'checker_cache':False}}
        for m in ('pairwise','shared')]
    if cfg['calls']!=calls:raise ValueError('fixed two-arm order/identity')
    asset=next(r['request'] for r in ledger['artifact_inventory'] if r['job_id']=='rank3_seed0_adaptive')
    ...
    if common['source_protocol']!={'path':'docs/residual_proof_protocol_20260925_r1.md',
            'sha256':expected['docs/residual_proof_protocol_20260925_r1.md']}:raise ValueError('protocol binding')
    ...
    output=ROOT/'data/moe/results/residual_proof_source4099_compare_20260925_r1'
    ...
    return {'audit':'PASS','issues':0,'config_sha256':sha(CONFIG),'implementation_commit':cfg['implementation_commit'],
        'source_files_checked':len(expected),'tests_passed':121,'dataset_index':4099,'model':'seed0',
