...
from residual_proof.contract import SCHEMA, MODES
...
DOC=ROOT/'docs/residual_proof_protocol_20260925_r1.md'
GATE=ROOT/'docs/residual_proof_controls_20260925_r1.json'
CONFIG=ROOT/'configs/backend_controls/residual_proof_compare_r1.json'
REVIEW=ROOT/'docs/residual_proof_freeze_review_20260925_r1.json'
DEST=ROOT/'data/moe/results/residual_proof_source4099_compare_20260925_r1'
...
def sources():
    ...
    paths=[DOC,*sorted((ROOT/'residual_proof').glob('*.py'))]
    return {**previous,**{str(p.relative_to(ROOT)):sha(p) for p in paths}}


def make_config():
    selection=load(SELECTION);ledger=load(LEDGER)
    model=selection['models']['seed0'];sample=next(s for s in selection['samples'] if s['sample_rank']==3)
    if (sample['dataset_index'],sample['label'])!=(4099,4):raise ValueError('fixed next-rank identity')
    old=next(r['request'] for r in ledger['artifact_inventory'] if r['job_id']=='rank3_seed0_adaptive')
    prior=load(ROOT/'configs/backend_controls/scoped_proof_execution_r1.json')
    scope={'experts':8,'classes':10,'label':4,'center':sample['center'],'model_state':model['model_state'],
    ...
    return {'schema':'RESIDUAL_PROOF_COMPARISON_R1','output':str(DEST),'common':common,
        'selection_files':{str(p.relative_to(ROOT)):sha(p) for p in (SELECTION,LEDGER)},
        'selection':'seed0 next manifest rank3 after sealed ranks0/1/2; no outcome/route predicate; observed development only',
        ...
        'route_margin_count':56,'route_candidate_files':{'pairwise':56,'shared':1},'proposal_stage_fraction':'1/2',
        ...
        'comparison':'same fixed route candidates, same retained source/output math; pairwise vs shared residual evidence',
        'baseline_limit':'both use checked router exclusions; remaining native infeasible LPs have no checked Farkas path and stay missing'}
...
def freeze():
    ...
    from residual_proof.controls import EXPECTED_TESTS
    ...
    save(CONFIG,cfg);print('FROZEN ONLY: CIFAR4099, two calls, 300s each, all252. ZERO real execution.')
