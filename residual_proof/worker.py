...
from residual_proof.contract import (accepted_routes, file_record, output_inputs,
                                    policy, retained_roster, route_inputs, route_roster, route_context)
...
def work(phase, root, deadline):
    ...
    if phase == 'route_propose':
        ...
        if mode == 'pairwise':
            ...
            rows = [events.call('fixed_shared_residual_potentials', lambda:
                propose(doc, pre, invocation=token, deadline=deadline))]
        expected = route_roster(scope)
        count = len(expected) if mode == 'pairwise' else 1
        prefix_hash = identity(pre)
        if len(rows) != count: raise ValueError('route proposal inventory')
        (root/'route_candidates').mkdir(exist_ok=False)
        files = {}
        for i,row in enumerate(rows):
            tick(deadline)
            record = {'schema':'RESIDUAL_ROUTE_FILE_V1','candidate':row,
                'context':route_context(spec,inv,digest,prefix_hash,i)}
            name=f'{i:04d}.json'
        ...
        events.call('serialize_route_completion',lambda:save(root/'route_complete.json',
            {'schema':'RESIDUAL_ROUTE_COMPLETE_V1','mode':mode,'invocation':token,
             'request_sha256':identity(scope),'source':file_record(root/'source.json'),
             'prefix':file_record(root/'router_prefix.json'),'files':files,
             'required_files':count,'required_margins':len(expected)}))
        ...
        result = events.call('independent_router_source_and_bounds', lambda: route_check(doc, pre, rows, mode=mode, invocation=token,
        ...
        events.call('serialize_route_check', lambda: save(root/'route_check.json',
            {'schema': 'RESIDUAL_ROUTE_CHECK_V1', 'context': receipt_context, 'frontier': result}))
        ...
        bundle = events.call('checked_lazy_source_construction', lambda: finish(doc, pre, rows, mode=mode, invocation=token,
            expected_source_sha256=bind_source(doc, scope), deadline=deadline))
        ...
        if phase == 'source_check':
            result = events.call('independent_retained_source_and_output_construction', lambda: check(doc, bundle, invocation=token,
                expected_source_sha256=bind_source(doc, scope), deadline=deadline))
        ...
        events.call('serialize_proposal_completion', lambda: save(root/'proposal_complete.json',
            {'schema': 'RESIDUAL_OUTPUT_COMPLETE_V1', 'invocation': token,
    ...
    if a.phase in ('route_check','source_check','aggregate'):
        ...
        def forbid(event, args):
            if event == 'import' and (args[0].split('.')[0] in ('torch','numpy','scipy','highspy','act','gurobipy') or
                    args[0] in ('checked_route_frontier.build','residual_proof.build','shared_route_residual.propose','scoped_source.build','full_source.obligations')):
