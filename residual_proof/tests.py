...
from residual_proof.contract import (accepted_routes, output_inputs, phases, policy,
                                     route_inputs, SCHEMA)
...
def fixture(root, mode='shared', prunable=True, bias=2.):
...
class Controls(unittest.TestCase):
    ...
    def test_complete_prunable_same_object_identical_matrices_both_arms(self):
        spec = copy.deepcopy(self.spec); spec['proof_policy']['mode'] = 'pairwise'
        other = self.work/'pairwise'
        # Both use the same strict router proofs, only their representation differs.
        ...
        self.assertEqual(new['pairs'], old['pairs'])
    ...
    def test_ties_keep_every_pair_and_property(self):
        ...
        spec=copy.deepcopy(spec);spec['proof_policy']['mode']='pairwise'
        other=self.work/'pairwise_ties'
        self.assertEqual(supervisor.supervise(other,spec,budget=30)['status'],POSITIVE)
    ...
    def test_fresh_solver_free_audit_and_no_overwrite(self):
        code=("import sys,json; from residual_proof.audit import audit; "
    ...
    def test_route_candidate_identity_and_complete_inventory(self):
        for key in ('invocation','source_sha256','prefix_sha256','mode','index'):
    ...
    def test_no_route_completion_is_partial_not_executable(self):
        ...
        self.assertEqual(rows['schema'],'SHARED_ROUTER_RESIDUAL_CERTIFICATE_V1');self.assertIsNone(context['completion'])
    ...
    def test_partial_route_then_exception_stops_before_construction(self):
        def dispatch(phase,folder,deadline):
            ...
            code=("from pathlib import Path; import residual_proof.worker as w; original=w.save\n"
                  "def stop(path,obj):\n if Path(path).name=='route_complete.json':raise RuntimeError('partial router publication')\n return original(path,obj)\n"
        ...
        report=audit(folder);self.assertEqual(report['route_bounds_received'],6);self.assertTrue(report['offline_router_recheck_only'])
    ...
    def test_partial_output_then_exception_cannot_promote(self):
        def dispatch(phase,folder,deadline):
            ...
            code=("from pathlib import Path;import scoped_proof.worker as p;import residual_proof.worker as w;original=p.propose;count=0\n"
    ...
    def test_late_positive_cannot_override_deadline(self):
        def dispatch(phase,folder,deadline):
            ...
            code=("import time;from pathlib import Path;from residual_proof.worker import work;"
    ...
    def batch(self, failed=False):
        ...
        if failed:
            rows[1]={'id':'shared','status':'SUPERVISOR_ERROR','seconds':.1,'launched':True,
    ...
    def test_batch_resource_refusals_keep_both_denominators(self):
        from residual_proof.batch_audit import review
    ...
    def test_batch_missing_duplicate_changed_terminal_rejected(self):
        ...
        terminal=load(root/'pairwise_batch_terminal.json');terminal['seconds']=0
        mutate(root/'pairwise_batch_terminal.json',terminal)
        with self.assertRaises(ValueError):review(root)
    ...
    def test_batch_supervisor_failure_is_explicit_audit_gap(self):
        ...
        self.assertFalse(report['shared_only_positive'])
    ...
    def test_complete_batch_same_source_and_cost_review(self):
        ...
        self.assertFalse(result['shared_only_positive']);self.assertGreater(result['observed_call_seconds'],0)
        # An accounting mutation cannot turn a 30-second control into a 300-second run.
        inv=load(root/'shared/invocation.json');inv['budget_seconds']=300
        mutate(root/'shared/invocation.json',inv)
        with self.assertRaises(ValueError):review(root)
    ...
    def test_accept_rejects_exclusion_and_guarantee_overclaim(self):
        for mode in ('zero','direction','coverage','native'):
            ...
            with self.assertRaises(ValueError):
                supervisor.accept(folder,self.spec['scope'],load(self.good/'invocation.json')['invocation'],'shared')

    def test_residual_run_binding_and_missing_source_layers(self):
        from residual_proof.check import check
        doc=load(self.good/'source.json');original=load(self.good/'construction.json')
        token=load(self.good/'invocation.json')['invocation']
        for mode in ('run','expert','pair','property','candidate_binding','route_mode'):
            bundle=copy.deepcopy(original)
            if mode=='run':bundle['invocation']='other'
            if mode=='expert':bundle['experts'].pop()
            if mode=='pair':bundle['pairs'].pop()
            if mode=='property':bundle['pairs'][0]['obligations']['rows'].pop()
            if mode=='candidate_binding':bundle['route_candidates']['binding']['invocation']='other'
            if mode=='route_mode':bundle['route_mode']='automatic'
            with self.subTest(mode=mode),self.assertRaises(ValueError):
                check(doc,bundle,invocation=token,expected_source_sha256=identity(doc),deadline=time.monotonic()+30)

    def test_cost_rss_terminal_and_receipt_mutations(self):
        for mode in ('rss','status','flag','schema'):
            folder=self.work/mode;shutil.copytree(self.good,folder)
            cost=load(folder/'cost.json')
            if mode=='rss':cost['sampled_peak_rss']=0
            if mode=='status':cost['status']='NOT_CLOSED';cost['complete_output_positive_proof']=False
            if mode=='flag':cost['complete_output_positive_proof']=False
            if mode=='schema':cost['schema']='OLD'
            mutate(folder/'cost.json',cost)
            with self.subTest(mode=mode),self.assertRaises(ValueError):audit(folder)
