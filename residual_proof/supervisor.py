...
from residual_proof.contract import phases, policy
...
def command(phase, root, deadline, spec):
    ...
    module = 'scoped_proof.worker' if phase == 'intake' else 'residual_proof.worker'
...
def accept(root, scope, token, mode):
    if mode not in ('pairwise','shared'): raise ValueError('mode')
...
def supervise(root, spec, *, budget=300., rss_limit=8*2**30, command_factory=None):
    ...
    terminal = {'schema':'RESIDUAL_PROOF_TERMINAL_V1','status_before_publication':status,
    ...
    save(root/'receipt.json',{'schema':'RESIDUAL_PROOF_RECEIPT_V1','invocation':token,
    ...
    save(root/'cost.json',{'schema':'RESIDUAL_PROOF_COST_V1','status':status,'invocation':token,
