...
from primitive_supervised.flow import supervise
from primitive_diagnostic import contract as C
from primitive_diagnostic.run import execute,summarize,audit_saved,launch_gate
from primitive_diagnostic.archive import comparison,partial,detail
...
class Controls(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        ARTIFACT_ROOT=Path(tempfile.mkdtemp(prefix='primitive_diagnostic_controls_',dir=ROOT/'data/moe/results'))
    ...
    def test_timeout_continues_then_exception_stops(self):
        ...
        from primitive_diagnostic import run
    ...
    def test_launch_gate_rejects_dirty_unpushed_and_wrong_branch(self):
        ...
        for failure in ('dirty','branch','remote','review'):
            ...
            with patch.object(C,'verify',return_value=v),patch.object(C,'ref',return_value={'fake':1}),\
                 patch('primitive_diagnostic.run.read',return_value=r),patch('scripts.optional_evidence_dev_contract.git',git):
    ...
    def test_freeze_rejects_policy_source_runtime_mutations(self):
        from primitive_diagnostic.contract import validate
        base={'schema':'PRIMITIVE_DIAGNOSTIC_FREEZE_V1','status':'FROZEN_NOT_EXECUTED',
        ...
        for name in ('policy','sources','output','runtime','status'):
            ...
            elif name=='sources':v[name]['primitive_supervised/flow.py']='changed'
