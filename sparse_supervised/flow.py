...
def sources():
    paths = [p for folder in ('sparse_supervised', 'sparse_basis', 'exact_primal')
    ...
    paths += [ROOT / p for p in ('lp_sandwich/check.py', 'lp_diagnostic/flow.py',
                                ...
                                'docs/sparse_basis_controls_attempt004.json',
...
def verify_sources(plan):
    if (plan['sources'] != sources() or plan['schema'] != 'SPARSE_BASIS_SUPERVISION_PLAN_V1' or
            plan['component_policy'] != POLICY or
    ...
    try:
        ...
        for name in STAGES:
            if name == 'check':
                ...
                cmd = [ACT, '-m', 'sparse_supervised.worker', name, str(root)]
...
def review_candidate(root, *, deadline=None):
    ...
    if 'capture' in names:
        ...
        if (r['schema'] != 'LARGE_NATIVE_BASIS_CAPTURE_V1' or r['policy'] != POLICY or
...
def supervise(spec, destination, *, started):
    ...
    save_new(root / 'plan.json', {'schema': 'SPARSE_BASIS_SUPERVISION_PLAN_V1', 'spec': spec, 'started': started,
                                 'total_seconds': 300, 'work_seconds': 298, 'proposal_seconds': 218,
                                 'component_policy': dict(POLICY), 'sources': sources()})
    ...
    try:
        ...
        with (root / 'driver.log').open('xb') as log:
            proc = subprocess.Popen([ACT, '-m', 'sparse_supervised.flow', str(root)], cwd=ROOT,
    ...
    if observed >= 300:
        status = 'TIMEOUT'
    terminal = {'schema': 'SUPERVISED_SPARSE_BASIS_V1', 'status': status,
...
def audit(root):
    ...
    verify_sources(p)
    if v['schema'] != 'SUPERVISED_SPARSE_BASIS_V1':
...
def raw_cost(root, raw, window):
    ...
    from lp_sandwich.check import identity
    from sparse_basis.native import OPTIONS
    v = read(root / 'prepared.json')
