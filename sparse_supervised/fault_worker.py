...
from sparse_supervised.worker import execute


def run(mode, root):
    if mode.startswith('drive_'):
        from sparse_supervised import flow
        ...
        def injected(folder, name, command, started, limit):
            if name == phase:
                command = [ACT, '-m', 'sparse_supervised.fault_worker', fault, str(folder)]
    ...
    if mode == 'native_stall':
        ...
        from sparse_basis import native
        ...
        with patch('sparse_basis.native.map_capture', fail):
            execute('map', root)
