...
class TransferFunction(ABC):
    ...
    def side_state_signature(self, layer_id: int) -> Any:
        """Optional abstract-state signature outside ``Fact.bounds``.

        Most transfer functions are fully represented by the returned
        ``Fact``. HybridZ also keeps HZ objects in a side cache; this hook lets
        the generic worklist notice cheap structural precision changes there.
        Returning ``None`` preserves the historical bounds-only behavior.
        """
        return None
