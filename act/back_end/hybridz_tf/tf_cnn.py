...
# (``HybridZonotope.py`` :6605-:6744). Solver-free:
#   1. Rearrange input HZ rows into pooling-window blocks.
#   2. Cheap interval-hull bounds per block (no LP).
#   3. If a block has a STABLE WINNER (``lb_winner >= ub_others`` for every other
#      entry), output the winner row exactly — full generator correlation kept.
#   4. Otherwise relax that block to ``[max(lb), max(ub)]`` with one fresh
#      independent continuous generator.
# Adds at most ``k_uns`` new continuous generators and 0 binary / 0 constraint
# rows. Same shape as DeepZ max-pool but tighter on the dominant-pixel case.
...
def _build_maxpool_index_map_chw(
    ...
    ``idx`` is a flat ``[num_blocks * block_size]`` Long tensor: entry ``i`` is
    the source row in the flattened CHW HZ for the ``i``-th block-position pair.
    ``-1`` entries represent padding positions (caller appends a zero pad row).
    """
...
def _bounds_box_hz(hz: HZono) -> Tuple[torch.Tensor, torch.Tensor]:
...
def hz_maxpool2d(
    ...
    Args:
        ...
        padding: int (max over a 4-tuple if given as 4-tuple, matching HyZor).
        input_shape: ``(C, H, W)`` or ``(1, C, H, W)``.

    Returns:
        HZono with ``n_out = C * H_out * W_out`` rows. Adds at most ``k_uns``
        (number of unstable blocks) new continuous generators, 0 new constraints.
    """
    ...
    lb_flat, ub_flat = _bounds_box_hz(rearranged)
