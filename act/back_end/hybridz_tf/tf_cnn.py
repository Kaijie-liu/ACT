...
def hybridz_tf_conv2d(L: Layer, Bin: Bounds) -> Fact:
    ...
    lb_conv = F.conv2d(Bin_lb, weight_pos, bias=None, stride=stride,
                       padding=padding, dilation=dilation, groups=groups)
    lb_conv += F.conv2d(Bin_ub, weight_neg, bias=None, stride=stride,
                        padding=padding, dilation=dilation, groups=groups)

    ub_conv = F.conv2d(Bin_ub, weight_pos, bias=None, stride=stride,
                       padding=padding, dilation=dilation, groups=groups)
    ub_conv += F.conv2d(Bin_lb, weight_neg, bias=None, stride=stride,
                        padding=padding, dilation=dilation, groups=groups)
...
def hybridz_tf_maxpool2d(L: Layer, Bin: Bounds) -> Fact:
    ...
    lb_pool = F.max_pool2d(Bin.lb.view(*input_shape), kernel_size, stride=stride, padding=padding)
    ub_pool = F.max_pool2d(Bin.ub.view(*input_shape), kernel_size, stride=stride, padding=padding)
    ...
    cons.add_op(f"maxpool2d:{L.id}", list(L.out_vars + L.in_vars), kernel_size=kernel_size,
        stride=stride, padding=padding, input_shape=input_shape, output_shape=L.params.get("output_shape"))
...
def hybridz_tf_avgpool2d(L: Layer, Bin: Bounds) -> Fact:
    ...
    lb_pool = F.avg_pool2d(Bin.lb.view(*input_shape), kernel_size, stride=stride, padding=padding)
    ub_pool = F.avg_pool2d(Bin.ub.view(*input_shape), kernel_size, stride=stride, padding=padding)

    Bout = Bounds(lb=lb_pool.view(-1), ub=ub_pool.view(-1))

    cons = ConSet()
    cons.add_op(f"avgpool2d:{L.id}", list(L.out_vars + L.in_vars), kernel_size=kernel_size, stride=stride,
        padding=padding, input_shape=input_shape, output_shape=L.params.get("output_shape"))
...
def hybridz_tf_flatten(L: Layer, Bin: Bounds) -> Fact:
    ...
    cons = ConSet()
    cons.add_op(f"flatten:{L.id}", list(L.out_vars + L.in_vars), start_dim=start_dim, end_dim=end_dim, 
...
def hybridz_tf_reshape(L: Layer, Bin: Bounds) -> Fact:
    ...
    cons = ConSet()
    cons.add_op(f"reshape:{L.id}", list(L.out_vars + L.in_vars), target_shape=target_shape, input_shape=L.params.get("input_shape"), 
output_shape=L.params.get("output_shape"))
