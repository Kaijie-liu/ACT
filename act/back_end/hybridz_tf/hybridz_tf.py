...
from act.back_end.solver.solver_hz import HZSolver, HZono
...
class HybridzTF(TransferFunction):
    ...
    def _hz_transform(self, L: Layer, hz_in: HZono) -> Optional[HZono]:
        ...
        if k == "DENSE":
            hz = s.hz_multiply(hz_in, L.params["weight"])
            b = L.params.get("bias")
            if b is not None:
                b_col = b.to(dtype=dtype, device=device)
                hz = s.hz_add_const(hz, b_col.view(-1, 1) if b_col.ndim == 1 else b_col)
            return hz
        if k == "BIAS":
            c = L.params["c"].to(dtype=dtype, device=device)
            return s.hz_add_const(hz_in, c.view(-1, 1) if c.ndim == 1 else c)
        if k == "SCALE":
            a = L.params["a"].to(dtype=dtype, device=device).flatten()
            return s.hz_multiply(hz_in, torch.diag(a))
        if k == "RELU":
            return s.hz_reduce(s.hz_apply_relu(hz_in))
        if k == "LRELU":
            return s.hz_reduce(s.hz_apply_leaky_relu(hz_in, float(L.params.get("negative_slope", 0.01))))
        if k == "TANH":
            return s.hz_apply_tanh(hz_in)
        if k == "SIGMOID":
            return s.hz_apply_sigmoid(hz_in)
        if k == "ABS":
            bds = s.hz_compute_bounds(hz_in)
            lb_out = torch.where(bds.lb >= 0, bds.lb, torch.where(bds.ub <= 0, -bds.ub, torch.zeros_like(bds.lb)))
            return s.hz_from_bounds(Bounds(lb=lb_out, ub=torch.maximum(bds.lb.abs(), bds.ub.abs())), dtype, device)
        if k == "CONV2D":
            return s.hz_conv2d(hz_in, L.params["weight"], L.params.get("bias"),
        ...
        if k == "ADD":
            ...
            return s.hz_minkowski_sum(hz_in, hz2) if hz2 is not None else None
        if k == "MUL":
            preds = self._net.preds.get(L.id, [])
            hz2 = self._hz_cache.get(preds[1]) if len(preds) > 1 else None
            if hz2 is not None:
                b1, b2 = s.hz_compute_bounds(hz_in), s.hz_compute_bounds(hz2)
                corners = torch.stack([b1.lb*b2.lb, b1.lb*b2.ub, b1.ub*b2.lb, b1.ub*b2.ub])
                return s.hz_from_bounds(Bounds(lb=corners.min(0)[0], ub=corners.max(0)[0]), dtype, device)
            return None
    ...
    def apply(self, L: Layer, input_bounds: Bounds, net: Net,
        ...
        if k in ("INPUT", "INPUT_SPEC"):
            hz_init = s.hz_from_bounds(input_bounds, input_bounds.lb.dtype, input_bounds.lb.device)
        ...
        if hz_in is not None:
            ...
            if hz_out is not None:
                ...
                hz_bounds = s.hz_compute_bounds(hz_out)
        ...
        if hz_in is not None and hz_bounds is None:
            self._hz_cache[L.id] = s.hz_from_bounds(
                fact.bounds, fact.bounds.lb.dtype, fact.bounds.lb.device)

        # Use HZ bounds if tighter
        if hz_bounds is not None:
            final = Fact(bounds=hz_bounds, cons=fact.cons)
        else:
            final = fact

        # Add box constraint for compute layers (consistent with IntervalTF)
        if k not in ("INPUT", "INPUT_SPEC", "ASSERT"):
            final.cons.add_box(L.id, L.out_vars, final.bounds)
