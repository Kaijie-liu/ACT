...
def sample_feasible_inputs(
    ...
    *,
    ...
    if in_cfg.kind == InKind.BOX:
        ...
        u = torch.rand((num_samples,) + shape, generator=g, device="cpu", dtype=torch.float64)
        x = lb.unsqueeze(0) + (ub - lb).unsqueeze(0) * u
        xs = [_move(x[i]) for i in range(num_samples)]
    ...
    if in_cfg.kind == InKind.LIN_POLY:
        ...
        if ab is None:
            A = torch.zeros((1, _prod(shape[1:])), device="cpu", dtype=torch.float64)
            b = torch.zeros((1,), device="cpu", dtype=torch.float64)
        else:
            A, b = ab
        n = _prod(shape[1:])
        lb_val = float(in_cfg.lb_val if in_cfg.lb_val is not None else in_cfg.value_range[0])
        ...
        tries = 0
        while len(xs) < num_samples and tries < int(max_tries):
            tries += 1
            u = torch.rand((1, n), generator=g, device="cpu", dtype=torch.float64)
            cand = lb_val + (ub_val - lb_val) * u
            ok = bool((A.matmul(cand.T) <= b.reshape(-1, 1) + 1e-12).all().item())
            if ok:
                cand = cand.reshape(shape)
                xs.append(_move(cand))
        status["tries_total"] = int(tries)
        status["accepted"] = int(len(xs))
        if tries > 0:
            status["rejection_rate"] = float(1.0 - (len(xs) / tries))
        if len(xs) < num_samples:
            status["input_sampling_status"] = "fallback"
            if strict_input:
                raise RuntimeError(
                    f"[input_sampling] rejection sampling failed: "
                    f"accepted={len(xs)}/{num_samples}, tries={tries}"
                )
