...
def _convert_OnnxResize(self, mod: nn.Module, node: fx.Node) -> None:
    ...
    # ONNX Resize positional inputs are (input, roi, scales, sizes). Resize-10
    # has (input, scales). Do not scan all constants here: roi is often a float
    # tensor and would be mistaken for scales.
    if len(args) >= 4:
...
def _test_resize_mode_and_dag_helpers() -> None:
    """Focused checks for Resize replay mapping and duplicate pred DAGs."""
    assert _onnx_resize_torch_mode("nearest", 2) == "nearest"
    assert _onnx_resize_torch_mode("linear", 1) == "linear"
    assert _onnx_resize_torch_mode("linear", 2) == "bilinear"
    assert _onnx_resize_torch_mode("linear", 3) == "trilinear"
    assert _onnx_resize_torch_mode("cubic", 2) == "bicubic"
    try:
        _onnx_resize_torch_mode("cubic", 3)
    except NotImplementedError:
        pass
    else:
        raise AssertionError("cubic/3D Resize must stay unsupported")

    # Mul(x, x) legitimately records the same predecessor twice for positional
    # semantics, while succs stores each graph edge once.  The DAG check must
    # reason over unique graph edges so it does not report a false cycle.
    _assert_dag({0: [], 1: [0, 0]}, {0: [1], 1: []}, 2)
