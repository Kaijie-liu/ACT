...
def _scurve_domain_cut_matrices(
    ...
    """Exact-valid cuts tying each S-curve segment binary to its x-domain.

    With segment binary zeta in {-1,+1}, zeta=-1 means selected.  The cuts are:

      x <= b        if selected, else x <= global_ub
      x >= a        if selected, else x >= global_lb

    They are redundant for integer segment binaries, but tighten the root LP.
    """
    owner_arr = np.asarray(owner_arr, dtype=np.int32).reshape(-1)
    r = int(owner_arr.size)
    if r == 0:
        return _empty(0, ng_total), _empty(0, nb_total), np.zeros(0, dtype=np.float64)
    ...
    if not (seg_a.size == seg_b.size == z_cols.size == r):
        raise ValueError("S-curve domain cut segment metadata size mismatch")
    ...
    g_lo = np.asarray(lb_w, dtype=np.float64).reshape(-1)[owner_arr]
    g_hi = np.asarray(ub_w, dtype=np.float64).reshape(-1)[owner_arr]
    center = hz.c[np.asarray(wide_idx, dtype=np.int64)[owner_arr]]
    rhs = np.empty(2 * r, dtype=np.float64)
    rhs[upper_rows] = (g_hi + seg_b) / 2.0 - center
    rhs[lower_rows] = -(g_lo + seg_a) / 2.0 + center

    # x - (global_ub - b)/2 * zeta <= (global_ub + b)/2
    # -x - (a - global_lb)/2 * zeta <= -(global_lb + a)/2
    ...
    pre_gb = hz.Gb[np.asarray(wide_idx, dtype=np.int64)].tocsr() if hz.n_bin else _empty(len(wide_idx), 0).tocsr()
    for j in range(len(wide_idx)):
        flats = np.nonzero(owner_arr == j)[0].astype(np.int32)
        if flats.size == 0:
            continue
        ...
        if c1 > c0:
            ...
            cols_rep = np.tile(cols, flats.size)
            data_rep = np.tile(data, flats.size)
            add_c(rows, cols_rep, data_rep)
            add_c(r + rows, cols_rep, -data_rep)
        if hz.n_bin:
            ...
            if b1 > b0:
                ...
                cols_rep = np.tile(cols, flats.size)
                data_rep = np.tile(data, flats.size)
                add_b(rows, cols_rep, data_rep)
                add_b(r + rows, cols_rep, -data_rep)

    Auc = sp.coo_matrix(
        (np.concatenate(dd_c), (np.concatenate(rr_c), np.concatenate(cc_c))),
        shape=(2 * r, ng_total),
    ).tocsr() if rr_c else _empty(2 * r, ng_total)
    Aub = sp.coo_matrix(
        (np.concatenate(dd_b), (np.concatenate(rr_b), np.concatenate(cc_b))),
        shape=(2 * r, nb_total),
    ).tocsr() if rr_b else _empty(2 * r, nb_total)
    Auc.eliminate_zeros()
    Aub.eliminate_zeros()
    return Auc, Aub, rhs
...
def _scurve_range_cut_matrices(
    ...
    """Exact-valid cuts tying each selected S-curve segment to its y-range."""
    owner_arr = np.asarray(owner_arr, dtype=np.int32).reshape(-1)
    r = int(owner_arr.size)
    if r == 0:
        return _empty(0, ng_total), _empty(0, nb_total), np.zeros(0, dtype=np.float64)
    ...
    if not (seg_y_lo.size == seg_y_hi.size == z_cols.size == r):
        raise ValueError("S-curve range cut segment metadata size mismatch")
    ...
    upper_rows = np.arange(r, dtype=np.int32)
    lower_rows = r + upper_rows
    g_lo = np.asarray(y_lo_w, dtype=np.float64).reshape(-1)[owner_arr]
    g_hi = np.asarray(y_hi_w, dtype=np.float64).reshape(-1)[owner_arr]
    center = out_c[np.asarray(wide_idx, dtype=np.int64)[owner_arr]]
    rhs = np.empty(2 * r, dtype=np.float64)
    rhs[upper_rows] = (g_hi + seg_y_hi) / 2.0 - center
    rhs[lower_rows] = -(g_lo + seg_y_lo) / 2.0 + center

    # y - (global_y_ub - segment_y_ub)/2 * zeta <= (global_y_ub + segment_y_ub)/2
    # -y - (segment_y_lb - global_y_lb)/2 * zeta <= -(global_y_lb + segment_y_lb)/2
...
def _scurve_graph_cut_matrices(
    ...
    *,
    ...
    """Conditional convex/concave graph cuts for sigmoid/tanh segments.

    Each segment is entirely on one side of the inflection.  For convex segments
    we add y <= secant and y >= endpoint tangents; for concave segments the
    inequalities are reversed.  The cuts are activated by the segment binary
    zeta=-1 and relaxed by an exact big-M computed from the neuron's global
    x/y interval, so they are redundant for the exact piecewise HZ encoding.
    """
    owner_arr = np.asarray(owner_arr, dtype=np.int32).reshape(-1)
    r = int(owner_arr.size)
    if r == 0:
    ...
    z_cols = np.asarray(z_cols, dtype=np.int32).reshape(-1)
    if not (seg_a.size == seg_b.size == z_cols.size == r):
        raise ValueError("S-curve graph cut segment metadata size mismatch")
    ...
    def append_linear_cut(seg_i: int, ax: float, ay: float, c0: float) -> None:
        owner = int(owner_arr[seg_i])
        max_g = (
            ax * (x_hi[owner] if ax >= 0.0 else x_lo[owner])
            + ay * (y_hi[owner] if ay >= 0.0 else y_lo[owner])
            + c0
        )
        M = max(0.0, float(max_g))
        row = len(rhs_vals)
        rhs_vals.append(M / 2.0 - ax * float(x_center[owner]) - ay * float(y_center[owner]) - c0)

        c0p, c1p = pre_gc.indptr[owner], pre_gc.indptr[owner + 1]
        add_c(row, pre_gc.indices[c0p:c1p], pre_gc.data[c0p:c1p], ax)
        b0p, b1p = pre_gb.indptr[owner], pre_gb.indptr[owner + 1]
        add_b(row, pre_gb.indices[b0p:b1p], pre_gb.data[b0p:b1p], ax)

        c0y, c1y = y_gc.indptr[owner], y_gc.indptr[owner + 1]
        add_c(row, y_gc.indices[c0y:c1y], y_gc.data[c0y:c1y], ay)
        b0y, b1y = y_gb.indptr[owner], y_gb.indptr[owner + 1]
    ...
    for s in range(r):
        ...
        if a < -1e-12 and b > 1e-12:
            # Construction should split at the inflection.  Skip if numerical
            # metadata ever violates that invariant.
            continue
    ...
    Auc = sp.coo_matrix(
        (np.concatenate(dd_c), (np.concatenate(rr_c), np.concatenate(cc_c))),
        shape=(n_rows, ng_total),
    ).tocsr() if rr_c else _empty(n_rows, ng_total)
    Aub = sp.coo_matrix(
        (np.concatenate(dd_b), (np.concatenate(rr_b), np.concatenate(cc_b))),
        shape=(n_rows, nb_total),
    ).tocsr() if rr_b else _empty(n_rows, nb_total)
    Auc.eliminate_zeros()
    Aub.eliminate_zeros()
