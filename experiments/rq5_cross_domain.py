...
Reproducible Run:
    python experiments/rq5_cross_domain.py --seed 42 --mode mock
...
def run_mock_domain_check(
    domain: str,
    net_seed: int,
) -> DomainResult:
    """
    Simulate domain comparison result for testing.

    Key behaviors:
    - interval: Widest bounds, fastest, lowest failure rate
    - hybridz:  Medium bounds, medium speed, medium failure rate
    - dual:     Tightest bounds, slowest, highest failure rate
    """
    detection_seed = derive_seed(net_seed, domain)
    set_all_seeds(detection_seed)

    config = {
        "interval": {"fail_prob": 0.45, "width_base": 2.0, "time_base": 5.0},
        "hybridz":  {"fail_prob": 0.55, "width_base": 1.7, "time_base": 12.0},
        "dual":     {"fail_prob": 0.68, "width_base": 1.5, "time_base": 25.0},
    }

    c = config.get(domain, config["interval"])

    bca_failed = torch.rand(1).item() < c["fail_prob"]
    bound_width = c["width_base"] + torch.randn(1).item() * 0.3
    bound_width = max(0.1, bound_width)
    time_ms = c["time_base"] + abs(torch.randn(1).item()) * c["time_base"] * 0.2

    return DomainResult(
        network_idx=0,
        network_seed=net_seed,
        domain=domain,
        bca_failed=bca_failed,
...
def run_rq5_experiment(
    ...
    for net_idx in range(num_networks):
        ...
        for domain in DOMAINS:
            result = run_mock_domain_check(domain, net_seed)
