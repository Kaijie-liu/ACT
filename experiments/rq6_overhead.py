...
from experiments._validation import (
    set_all_seeds,
    derive_seed,
    ExperimentMetadata,
)
...
def create_model(input_dim: int, hidden_sizes: List[int], output_dim: int) -> torch.nn.Module:
    """Create a PyTorch model for timing measurement."""
    layers = []
    prev_size = input_dim
...
def measure_scc_overhead(
    ...
    # Create model
    model = create_model(input_dim, [64, 32], 10)
    model.eval()
...
def run_rq6_experiment(
    ...
    for size_name, size_config in model_sizes.items():
        ...
        # Create model
        model = create_model(input_dim, hidden, output_dim)
        model.eval()
        ...
        for budget in sampling_budgets:
            ...
            for _ in range(warmup_runs):
                measure_scc_overhead(input_dim, budget, seed=experiment_seed)

            # Timing runs
            for run_idx in range(timing_runs):
                run_seed = derive_seed(experiment_seed, size_name, budget, run_idx)
                times = measure_scc_overhead(input_dim, budget, seed=run_seed)
        ...
        # Warmup
        for _ in range(warmup_runs):
            measure_bca_overhead(model, layer_bounds, input_dim, seed=experiment_seed)

        # Timing runs
        for run_idx in range(timing_runs):
            run_seed = derive_seed(experiment_seed, size_name, "bca", run_idx)
            times = measure_bca_overhead(model, layer_bounds, input_dim, seed=run_seed)
    ...
    latex_table = generate_latex_table_rq6(results["summary"], model_sizes, sampling_budgets)
...
def generate_latex_table_rq6(
    ...
    for size_name in size_names:
        label = size_labels.get(size_name, size_name.capitalize())
        bca_key = f"bca_{size_name}"
        bca_ms = summary.get(bca_key, {}).get("avg_total_ms", 0.0)

        # Get params from first budget entry
        first_scc_key = f"scc_{size_name}/budget{budgets[0]}"
        # Estimate params from model config
        ...
        params = f"$\\sim${n_params // 1000}K" if n_params >= 1000 else f"$\\sim${n_params}"

        cbr_vals = []
        bbl_vals = []
        comp_vals = []
        for b in budgets:
            scc_key = f"scc_{size_name}/budget{b}"
            scc_ms = summary.get(scc_key, {}).get("avg_total_ms", 0.0)
            cbr_vals.append(f"{scc_ms:.2f}")
            bbl_vals.append(f"{bca_ms:.2f}")
            comp_vals.append(f"{scc_ms + bca_ms:.2f}")

        cbr_str = " & ".join(cbr_vals)
        bbl_str = " & ".join(bbl_vals)
        comp_str = " & ".join(comp_vals)

        lines.append(f"{label} & {params} & {cbr_str} & {bbl_str} & {comp_str} \\\\")
