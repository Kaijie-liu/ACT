...
from reuse_supervised.study import verify,summarize,OUTPUT,FREEZE,REVIEW

DEST=ROOT/'docs/reuse_supervised_v1_execution_results.json'
...
def aggregates(rows,indices):
    ...
    for index in indices:
        ...
        if set(pair)!={'reuse_off','reuse_on'}:raise ValueError('missing arm')
        ...
        pairs.append({'dataset_index':index,'off_seconds':b['wall_seconds'],'on_seconds':a['wall_seconds'],
                      'on_minus_off_seconds':a['wall_seconds']-b['wall_seconds'],
    ...
    return {'by_arm':arms,'paired':pairs,
            'paired_cost_median_seconds':statistics.median(r['on_minus_off_seconds'] for r in pairs),
            'total_request_seconds':sum(r['wall_seconds'] for r in rows),
            'on_vs_off_total_reduction_fraction':1-arms['reuse_on']['total_whole_seconds']/arms['reuse_off']['total_whole_seconds']}
...
def source_compare(manifests,roots):
    ...
    for key in a['contexts']:
        ...
        for kind in ('joint_source','router_source'):
            ...
            item[kind]={'on_sha256':refs[0]['sha256'],'off_sha256':refs[1]['sha256'],
...
def review():
    ...
    for i,sample in enumerate(v['samples']):
        names=[f'rank{i}_{arm}' for arm in ('reuse_on','reuse_off')]
    ...
    return {'schema':'UPSTREAM_REUSE_ARCHIVE_V1','status':'PASS','issues':[],
            ...
            'timing_caveat':'nested exclusive categories are observations; combined source+matrix ablation does not establish either component alone suffices'}
