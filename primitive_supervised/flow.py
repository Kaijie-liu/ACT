...
from primitive_basis.engine import POLICY
from sparse_basis.engine import POLICY as NATIVE_POLICY
from primitive_supervised.journal import JOURNAL_POLICY
...
def sources():
    paths = [p for folder in ('primitive_supervised', 'primitive_basis', 'sparse_basis', 'exact_primal')
             for p in (ROOT / folder).glob('*.py')]
    paths += [ROOT / p for p in ('fidelity_supervised/native.py', 'docs/primitive_basis_controls_attempt002.json', 'docs/primitive_basis_v1_review.json', 'lp_sandwich/check.py', 'lp_diagnostic/flow.py',
...
def verify_sources(plan):
    if (plan['sources'] != sources() or plan['schema'] != 'PRIMITIVE_BASIS_SUPERVISION_PLAN_V1' or
    ...
    try:
        ...
        for name in STAGES:
            if name == 'check':
                ...
                cmd = [ACT, '-m', 'primitive_supervised.worker', name, str(root)]
...
def review_candidate(root, *, deadline=None):
    ...
    if 'construct' in names:
        ...
        if (construction['schema'] != 'TIMED_PRIMITIVE_BASIS_PROPOSAL_V1' or construction['policy'] != POLICY or
...
def supervise(spec, destination, *, started):
    ...
    save_new(root / 'plan.json', {'schema': 'PRIMITIVE_BASIS_SUPERVISION_PLAN_V1', 'spec': spec, 'started': started,
    ...
    try:
        ...
        with (root / 'driver.log').open('xb') as log:
            proc = subprocess.Popen([ACT, '-m', 'primitive_supervised.flow', str(root)], cwd=ROOT,
    ...
    if observed >= 300:
        status = 'TIMEOUT'
    terminal = {'schema': 'SUPERVISED_PRIMITIVE_BASIS_V1', 'status': status,
...
def audit(root):
    ...
    verify_sources(p)
    if v['schema'] != 'SUPERVISED_PRIMITIVE_BASIS_V1':
