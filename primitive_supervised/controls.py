...
from primitive_supervised.flow import sources,audit,costs


def old():
    from primitive_basis.controls import old as sealed,sources as component_sources
    sealed()
    c=read(ROOT/'docs/primitive_basis_controls_attempt002.json')
    if c['status']!='PASS' or c['sources']!=component_sources():raise ValueError('frozen primitive drift')


def run():
    old();before=sources();n=1
    while (ROOT/f'docs/primitive_supervised_controls_attempt{n:03}.json').exists():n+=1
    ...
    result=unittest.TextTestRunner(stream=log,verbosity=2).run(unittest.defaultTestLoader.loadTestsFromNames([
        'primitive_supervised.tests','fidelity_supervised.native_tests',
    ...
    from primitive_supervised.tests import OBSERVATIONS,ARTIFACT_ROOT
    ...
    dest=ROOT/f'docs/primitive_supervised_controls_attempt{n:03}.json'
