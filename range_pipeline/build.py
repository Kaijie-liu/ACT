...
CODE={**OLD_CODE,'range_pipeline/verify_source.py':'verify_full.py',
...
def build(root,job,deadline):
    ...
    arm=job['arm']
    if arm not in ('range_off','range_on'):raise ValueError('fixed arm')
    ...
    policy={'enabled':arm=='range_on','layer':6,'row_prefix':2,'native_seconds':3.}
    ...
    for e in doc['experts']:
        ...
        for index in ['coordinate']+list(range(2,9)):
            t=time.monotonic();old=state;before=shape;range_seconds=0.
            ...
            if index==6:
                ...
                for j in range(min(2,len(op))):
                    if policy['enabled']:
                        fact,r=propose(old,op[j],bias[j],context,j,range_dir,f'e{i}_r{j}',deadline,3.)
                        facts[j]=fact
                    else:r={'context':context,'row':j,'source_sha256':identity(old),'calls':0,'outcome':'DISABLED','fact_sha256':None,'seconds':0.}
            ...
            times.append({'expert':i,'layer':index,'seconds':time.monotonic()-t,'range_seconds':range_seconds,
                'propagation_seconds':propagation,'delta_serialization_seconds':serial,
    ...
    save(dst/'trace.json',{'schema':'RANGED_COMPLETE_EXPERT_TRACE_V1','steps':steps,'joint_proof':proof,'joint_sha256':identity(joint)})
    ...
    save(dst/'manifest.json',{'schema':'RANGED_COMPLETE_EXPERT_LP_BUNDLE_V1','policy':policy,
