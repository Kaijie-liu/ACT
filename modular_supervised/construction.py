...
from modular_basis.engine import POLICY, statistics, eliminate, Unresolved
from modular_supervised.journal import TimedBudget


def propose(lp, statement, candidate, hint, expected_statement, expected_hint, *, deadline, root, original_started):
    begin = time.monotonic()
    b = TimedBudget(deadline, root, begin=begin, original_started=original_started,
                    bindings={'lp_sha256': identity(lp), 'statement_sha256': expected_statement,
                              'hint_sha256': expected_hint})
    ...
    return {'schema': 'TIMED_MODULAR_BASIS_PROPOSAL_V1', 'status': status, 'error': error,
            'bundle': bundle, 'row_residuals': residuals, 'stats': stats, 'operations': b.operations,
            'policy': dict(POLICY), 'arithmetic': b.arithmetic, 'seconds': seconds, 'journal': b.summary(seconds), 'deadline_monotonic': deadline,
